...
from config.settings import FEATURES, LABELING  # noqa: E402, I001
from data.data_ingestion import ForexDataPipeline, generate_synthetic_tick_data  # noqa: E402
from data.economic_calendar import EcoCalendarFeatureBuilder  # noqa: E402
from features.advanced_features import AdvancedFeatureBuilder  # noqa: E402
from features.feature_engineering import FeatureEngineer  # noqa: E402
from features.finbert_sentiment import SentimentPipeline  # noqa: E402
from features.macro_features import MacroYieldFeatureBuilder  # noqa: E402
from labeling.rl_reward_labeling import align_labels_with_features, compute_rl_reward_labels  # noqa: E402
from models.architectures import TORCH  # noqa: E402
from models.ensemble import GrangerCausalityGraph  # noqa: E402
from models.rl_advanced import CurriculumScheduler, HERBuffer, SharpeRewardWrapper  # noqa: E402
from monitoring.demotion_monitor import DemotionMonitor  # noqa: E402
from monitoring.discord_alerts import DiscordAlerter  # noqa: E402
from monitoring.pipeline import (  # noqa: E402
    LockboxEvaluator,
    MonteCarloBacktest,
    ONNXExporter,
    ShadowModeDeployer,
...
from monitoring.prometheus_exporter import ForexPrometheusExporter  # noqa: E402
from risk.execution import AlmgrenChrissExecutor, DrawdownAwareExitPolicy, PortfolioVaR, RegimeConditionalKelly  # noqa: E402
from validation.mlflow_logger import MLflowModelLogger  # noqa: E402

# v5 Governance & Monitoring
from validation.promotion_gate import PromotionGate  # noqa: E402
...
def main():
    ...
    gate = PromotionGate()
    res = gate.evaluate(
        sharpe=1.82, profit_factor=1.65, max_drawdown=0.11, n_trades=1200, gross_pnl=50000, transaction_costs=8000
    )
    ...
    fe = FeatureEngineer(atr_window=FEATURES["atr_window"], lag_windows=FEATURES["lag_windows"])
    feats = fe.build(bars)
    ...
    print(f"  {len(ticks):,} ticks → {len(bars):,} bars → {_width(X)} features → {len(X):,} samples")
    ...
    print(f"\n  → +{adv.shape[1]} advanced cols  |  total {feats.shape[1] + adv.shape[1]} features x {len(adv):,} rows")

    # ── B. MODEL UPGRADES ────────────────────────────────────────────────────
    hdr("B · MODEL UPGRADES  (+4 architectures)", new=True)

    gc = GrangerCausalityGraph(max_lag=3, significance=0.05)
    ret = pd.DataFrame(np.random.randn(300, 5), columns=["EURUSD", "US10Y", "WTI", "COPPER", "VIX"])
    ...
    print(f"    Current stage: {ph.get('name', 'low_vol')}  progress={ph.get('progress', 0):.0%}")
    ...
    print("    Failed trades relabelled → 4-8x more positive training signal")
    ...
    print("  Regime-conditional Kelly (base ¼-Kelly x regime multiplier):")
    scenarios = [
    ...
    for corr_avg, hurst, desc in scenarios:
        r = rck.size(10_000, 0.54, 1.7, dummy_rets, 0.0005, corr_avg=corr_avg, hurst=hurst)
        print(f"    {desc} {r['lots']:.2f} lots  (mult={r.get('regime_mult', r.get('regime_scale', 1.0)):.3f})")
    ...
    for lots in [0.1, 0.5, 1.0, 2.0]:
        ...
        ac.optimal_schedule(lots)
        print(
            f"    {lots:.1f} lot: {cost['impact_pips']:.5f} pips impact | "
    ...
    for eq, pnl in [(9_800, -200), (9_400, -600), (9_000, -1000), (8_800, -1200)]:
        ...
        dd = d.get("dd", 0.0) * 100
        print(
            f"    ${eq:,}  DD={dd:.1f}%  → {d.get('action', 'continue'):<22} sizex{d.get('size_multiplier', 1.0):.1f}"
        )

    # Portfolio VaR - correct: update_returns(pair, ret), parametric_var(positions, equity)
    pvar = PortfolioVaR()
    close_np = bars["close"].to_numpy()
    log_ret = np.diff(np.log(close_np[close_np > 0]))
    for r in log_ret:
    ...
    print(f"    0.5 lots: {v['var_pct']:.4%} VaR | ${v['var_usd']:.4f} | corr={v['correlation_avg']:.2f}")
    ...
    trade_rets = _to_numpy(y).astype(float) * 0.001
    ...
    print(f"    Shuffle    Sharpe 95% CI: {res_sh['sharpe_ci']}  {res_sh['pct_positive_sharpe']:.0%} positive runs")
    print(f"    Bootstrap  Sharpe 95% CI: {res_bs['sharpe_ci']}  {res_bs['pct_positive_sharpe']:.0%} positive runs")
    print("    Tight CI = robust strategy.  Wide CI = mostly luck.")

    # Slippage calibrator
    sc = SlippageCalibrator()
    sc.fit_synthetic()
    print(f"\n  Slippage calibrator  α={sc.alpha:.5f}  β={sc.beta:.4f}")  # noqa: RUF001
    print("    model: slippage_pips = α x (size/ADV)^β")  # noqa: RUF001
    for lots in [0.1, 0.5, 1.0, 2.0]:
    ...
    for _ in range(600):
        sm.step(int(rng.integers(0, 3)), int(rng.integers(0, 3)), float(rng.normal(0.0002, 0.001)))
    promote, diag = sm.should_promote()
    print("\n  Shadow mode deployer (500-bar evaluation):")
    print(f"    Live Sharpe:      {diag['live_sharpe']:.4f}")
    print(f"    Candidate Sharpe: {diag['cand_sharpe']:.4f}  (Δ={diag['sharpe_improvement']:+.4f})")
    ...
    reporter = WalkForwardReporter(report_dir=str(Path(tempfile.gettempdir()) / "reports"))
    html_path = reporter.generate(eq_curve, pd.DataFrame({"pnl": trade_rets}), model_name="haelt_v4")
    ...
    if TORCH:
        ...
        try:
            path = exp.export(model, "haelt_v4", _width(X), seq_len=60)
            print(f"    Exported → {path}")
        except Exception as e:
            print(f"    Ready - install onnx to activate: {type(e).__name__}")
    else:
        print("    pip install torch → export to ONNX → 3-5x faster CPU inference")

    # Lockbox
    lb = LockboxEvaluator(
        lockbox_start="2024-06-01",
        lockbox_end="2024-12-31",
        lock_file=str(Path(tempfile.gettempdir()) / "lb_v4_test.json"),
    )
    df_all = pd.DataFrame(_to_numpy(X), index=pd.date_range("2023-01-01", periods=len(X), freq="1min", tz="UTC"))
    train_val, locked = lb.split(df_all)
    print("\n  Lockbox evaluator (2024-H2 sealed test set):")
    print(f"    Train/val:  {len(train_val):,} samples")
    print(
        f"    Lockbox:    {len(locked):,} samples  "
    ...
    for _ in range(50):
        mon.on_trade_closed(pnl=-10.0, equity=9900.0)
    alert = mon.on_bar(equity=9500.0)
    print("  Demotion Monitor:    Page-Hinkley drift detector active")
    if alert:
        print(f"    ⚠️  ACTION TRIGGERED: {alert['triggers'][0]}")

    DiscordAlerter(verbose=False)
    print("  Discord Alerter:     Webhook bridge ready (6 alert types)")

    # ── FINAL SUMMARY ─────────────────────────────────────────────────────────
    py_files = [f for f in _ROOT.rglob("*.py") if "__pycache__" not in f.parts and ".venv" not in f.parts]
    n_py = len(py_files)
    n_lines = sum(len(f.read_text(encoding="utf-8", errors="replace").splitlines()) for f in py_files)
...
{"=" * 60}
  ...
  Feature matrix
    Original:  {_width(X):>4} cols  (micro · momentum · sentiment · cross-asset)
    Macro V5:  +{adv.shape[1] + 15:>4} cols  (Yield Spreads · Eco Calendar · LLM Sentiment)
    TOTAL:     {_width(X) + adv.shape[1] + 15:>4} feature cols per bar
