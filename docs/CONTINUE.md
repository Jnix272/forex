...
> ✅ **2026-08-17:** **Checkpoint-load report contract fix** verified. The helper in `training/model_factory.py` now returns a dict summary (`frac_loaded`, `n_loaded`, `n_target`, `missing`, `unexpected`, `shape_mismatch`) instead of a raw `(missing, unexpected)` tuple, matching the callers in `training/supervised_loop.py`. This removes the `.get(...)` tuple-attribute error. Verification: `python -m pyright training/supervised_loop.py training/model_factory.py` → **0 errors**.
