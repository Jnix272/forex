...
class PortfolioVaR:
    ...
    (lots × $/pip) by a covariance of price-fraction returns (units of price²),
    yielding wildly wrong dollar magnitudes ($0.0096 instead of $96.48 for a
    standard EURUSD lot at 99% confidence). By moving to notional × price-fraction
    returns throughout, units are consistent: ($notional) × price-fraction ×
    price-fraction → $² variance, and VaR comes out in $.
    """

    DEFAULT_NOTIONAL_PER_LOT = 100_000.0  # standard FX lot

    def __init__(self, confidence=0.99, horizon=1, pip_value=10.0,
                 max_var_pct=0.02, notional_per_lot=None):
    ...
    def parametric_var(self, positions, equity):
        ...
        # FIX: dollar notional weights (units: $/lot × lots = $).
        # Notional × price-fraction gives $ per unit move; consistent with cov
        ...
        return {"var_pct": round(var / max(equity, 1), 6),
                "var_usd": round(var, 4), "cvar_usd": round(cvar, 4),
    ...
    def max_allowed_lots(self, pair, equity, positions, pip_value=None):
        ...
        in dollar-notional × price-fraction units; see class docstring).
        """
        budget = equity * self.max_var
        min_obs = 20

        # Existing positions with sufficient return history.
        exist_pairs = [p for p in positions
                        if abs(positions[p]) > 0.001 and len(self._returns.get(p, [])) >= min_obs]
