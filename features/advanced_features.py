...
def synthetic_orderbook(bars, n_levels=10):
    ...
    return {
        "bid_levels": mid.reshape(-1, 1) - sp,
        "ask_levels": mid.reshape(-1, 1) + sp,
...
def order_book_features(ob, index, k=5):
    ...
    bl = ob["bid_levels"]
    al = ob["ask_levels"]
    F["ob_bid_depth"] = bv.sum(1)
    F["ob_ask_depth"] = av.sum(1)
...
def session_features(index):
    ...
    tm = h * 60 + m
    F["tod_sin"] = np.sin(2 * np.pi * tm / 1440)
    F["tod_cos"] = np.cos(2 * np.pi * tm / 1440)
    F["dow_sin"] = np.sin(2 * np.pi * index.dayofweek / 5)
    F["dow_cos"] = np.cos(2 * np.pi * index.dayofweek / 5)
    return F
...
def correlation_regime_features(returns_df, window=60):
    ...
    for i in range(nc):
        for j in range(i + 1, nc):
            pair_corrs.append(returns_df[cols[i]].rolling(window, min_periods=min_periods).corr(returns_df[cols[j]]))
    ...
    rm = F["corr_avg"].rolling(window * 3).mean()
...
def fast_trend_score(high: pd.Series, low: pd.Series, close: pd.Series, window: int = 20) -> pd.Series:
    ...
    """
    low_s: pd.Series = pd.Series(low, dtype=float).clip(lower=1e-12)
    high_s: pd.Series = pd.Series(high, dtype=float).clip(lower=1e-12)
    close_s: pd.Series = pd.Series(close, dtype=float).clip(lower=1e-12)
    log_hl: pd.Series = pd.Series(np.log(high_s / low_s), index=high_s.index, name="log_hl")
    parkinson = np.sqrt((log_hl.pow(2).rolling(window, min_periods=2).mean()) / (4 * np.log(2)))
    prev = close_s.shift(1).clip(lower=1e-12)
    cc: pd.Series = pd.Series(np.log(close_s / prev), index=close_s.index, name="log_ret")
...
def fractal_dimension(series, window=30):
    ...
    for i in range(window, n):
        ...
        for k in range(1, window // 2):
            ...
            if segs:
                Lm.append(np.mean(segs) * (window - 1) / (len(segs) * k * k))
        if len(Lm) > 2:
            try:
...
class AdvancedFeatureEngineer:
    ...
    def build(self, bars: pl.DataFrame, base_features: pl.DataFrame, cot_data=None) -> pl.DataFrame:
        ...
        idx = base_pd.index
        bars_a = bars_pd.reindex(idx).ffill()
        F = pd.DataFrame(index=idx)
        ob = synthetic_orderbook(bars_a)
        F = pd.concat([F, order_book_features(ob, idx)], axis=1)
        ...
        if ret_cols:
            F = pd.concat([F, correlation_regime_features(base_pd[ret_cols], self.cw)], axis=1)
        close = bars_a["close"]
        tr = fast_trend_score(bars_a["high"], bars_a["low"], close, window=max(10, min(self.hw, 50)))
        close_series = pd.Series(close, dtype=float)
        log_ret = pd.Series(
            np.log(close_series.clip(lower=1e-12) / close_series.shift(1).clip(lower=1e-12)),
            index=close.index,
            name="log_ret",
        ).fillna(0.0)
...
def rolling_hurst_fractal(bars, windows=None):
    import pandas as pd
...
class AdvancedFeatureBuilder:
    ...
    def __init__(self, hurst_windows=None, **kw):
        if hurst_windows is None:
            hurst_windows = [30, 60, 120]
...
# ─────────────────────────────────────────────────────────────────────────────
# MULTI-PAIR INTERACTION FEATURES  (B7–B11)  # noqa: RUF003
...
def compute_asia_london_gap(bars: pl.DataFrame, atr: pl.Series | None = None) -> pl.Series:
    time_col = (
        "timestamp_utc"
    ...
    gap_series: pl.Series = df["gap"].forward_fill()
