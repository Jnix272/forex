...
YIELD_DEFAULTS = {
    "US10Y": 4.50, "US2Y":  4.20, "DE10Y": 2.40, "JP10Y": 0.80,
    "GB10Y": 4.20, "AU10Y": 4.40, "CA10Y": 3.80, "NZ10Y": 4.60,
    "CH10Y": 0.90,
}
...
def _synthetic_yields(
    ...
    # 9×9 correlation matrix (order matches _YIELD_NAMES)
    corr = np.array([
    ...
    return {name: pd.Series(paths[:, i], index=days)
            for i, name in enumerate(_YIELD_NAMES)}
...
class MacroYieldFeatureBuilder:
    ...
    def build(self, bars: pl.DataFrame) -> pl.DataFrame:
        ...
        start = pd.Timestamp(min_dt) - pd.Timedelta(days=35)
        ...
        bars_tz = bars["timestamp_utc"].dtype.time_zone
        if bars_tz is None:
        ...
        yield_df = yield_df.with_columns(pl.col("timestamp_utc").cast(bars["timestamp_utc"].dtype))

        # Join asof
        out = bars.select(["timestamp_utc"]).sort("timestamp_utc").join_asof(
            yield_df.sort("timestamp_utc"),
            on="timestamp_utc",
            strategy="backward"
        )
        ...
        # ── Primary spread momentum + volatility (USD−EUR as primary) ─────────
        exprs = []
