...
def add_cot_features(df: pl.DataFrame, cot_df: pl.DataFrame, pair: str) -> pl.DataFrame:
    ...
    pair_cot = pair_cot.with_columns([
        ...
        ((pl.col("net_hedge_fund") - pl.col("net_hedge_fund").rolling_mean(52)) / 
         (pl.col("net_hedge_fund").rolling_std(52) + 1e-9)).alias("cot_hf_zscore_52w")
    ])
    ...
    df = df.with_columns([
        pl.when(pl.col("cot_hf_zscore_52w") > 2.0).then(1.0)
          .when(pl.col("cot_hf_zscore_52w") < -2.0).then(-1.0)
          .otherwise(0.0).alias("cot_extreme")
    ])
