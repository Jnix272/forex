...
from feature_store.registry import get_registry
...
class FeatureMaterializer:
    ...
    def __init__(
        ...
        if lineage_dir:
            ...
            self.lineage_tracker = LineageTracker(store=FileLineageStore(Path(lineage_dir) / "lineage.jsonl"))
    ...
    def _load_raw_data(
        ...
        try:
            ...
            return mgr.load(
                pair, source=source, start=start_date.strftime("%Y-%m-%d"), end=end_date.strftime("%Y-%m-%d")
            )
        except Exception as e:
    ...
    def materialize_incremental(
        ...
        if base_features is not None and len(base_features) > 0:
            ...
            base_features = base_features.filter(~pl.col("timestamp_utc").is_in(new_timestamps))
        ...
        start_value = new_bars["timestamp_utc"].min()
        end_value = new_bars["timestamp_utc"].max()
        start_dt = start_value if isinstance(start_value, datetime) else datetime.fromisoformat(str(start_value))
        end_dt = end_value if isinstance(end_value, datetime) else datetime.fromisoformat(str(end_value))
...
def create_materializer(
    feature_store_path: str | Path = "./feature_store", feature_engineer: FeatureEngineer | None = None, **kwargs
) -> FeatureMaterializer:
    """Create a feature materializer"""
    return FeatureMaterializer(feature_store_path=feature_store_path, feature_engineer=feature_engineer, **kwargs)
