...
class FeatureVersion:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "FeatureVersion":
        return cls(
            ...
            n_cols=data["n_cols"],
            pairs=data["pairs"],
            date_range=tuple(datetime.fromisoformat(d) for d in data["date_range"]) if data.get("date_range") else None,
    ...
    def _load_metadata(self):
        ...
        if self._metadata_file.exists():
            with open(self._metadata_file) as f:
                data = json.load(f)
                self.versions = {
                    k: FeatureVersion.from_dict(v) for k, v in data.items()
                }
        else:
            self.versions = {}
    ...
    def write(
        ...
        with self._lock:
            ...
            if "timestamp_utc" in features.columns:
                ts_min = features["timestamp_utc"].min()
                ts_max = features["timestamp_utc"].max()
...
class DeltaFeatureStore(FeatureStore):
    ...
    def write(self, features: pl.DataFrame, version: str, pair: str | None = None, 
              description: str = "", tags: dict[str, str] | None = None, lineage: dict | None = None) -> FeatureVersion:
        raise NotImplementedError()
    
    def read(self, version: str, pair: str | None = None, start_date: datetime | None = None,
             end_date: datetime | None = None, columns: list[str] | None = None) -> pl.DataFrame:
        raise NotImplementedError()
    
    def read_latest(self, pair: str | None = None, start_date: datetime | None = None,
                    end_date: datetime | None = None, columns: list[str] | None = None) -> tuple[pl.DataFrame, FeatureVersion]:
        raise NotImplementedError()
...
def create_feature_store(
    ...
    base_path: str | Path = "./feature_store",
    **kwargs
) -> FeatureStore:
