...
from contracts.base import Stage
...
from data.data_ingestion import ForexDataPipeline, generate_synthetic_tick_data, load_or_generate
...
from features.incremental import StreamingFeatureProcessor
...
class PipelineOrchestrator:
    ...
    def __init__(
        ...
        self.drift_detector = SchemaDriftDetector(reference_dir=self.output_dir / "drift_reference")
        ...
        if self.config.quality_gates.enabled:
            self.quality_gates = {
                ...
                "feature_engineering": create_quality_gates(
                    "feature_engineering", remediation_log_dir=self.config.quality_gates.log_dir
                ),
            }
        else:
            self.quality_gates = {}
    ...
    def _init_components(self):
        ...
        self.data_pipeline = ForexDataPipeline(
            ...
            info_bar_threshold=(
                self.config.bars.tick_threshold
                if self.config.bars.bar_type.value == "tick"
                else self.config.bars.volume_threshold
                if self.config.bars.bar_type.value == "volume"
    ...
    def run(self, pairs: list[str] | None = None) -> ValidationReport:
        ...
        if self.config.feature_store.enabled:
            self.stages[PipelineStageName.MATERIALIZATION] = PipelineStage(name=PipelineStageName.MATERIALIZATION)

        if self.config.validation.drift_detection_enabled:
            self.stages[PipelineStageName.VALIDATION] = PipelineStage(name=PipelineStageName.VALIDATION)
        ...
        try:
            ...
            for pair in pairs:
                ...
                # Stage 1: Ingestion
                self._run_stage(PipelineStageName.INGESTION, pair, lambda: self._stage_ingestion(pair))  # noqa: B023

                # Stage 2: Resampling
                self._run_stage(PipelineStageName.RESAMPLING, pair, lambda: self._stage_resampling(pair))  # noqa: B023

                # Stage 3: Feature Engineering
                self._run_stage(PipelineStageName.FEATURE_ENGINEERING, pair, lambda: self._stage_features(pair))  # noqa: B023

                # Stage 4: Labeling
                self._run_stage(PipelineStageName.LABELING, pair, lambda: self._stage_labeling(pair))  # noqa: B023

                # Stage 5: Dataset Build
                self._run_stage(PipelineStageName.DATASET_BUILD, pair, lambda: self._stage_dataset(pair))  # noqa: B023
    ...
    def _run_stage(self, stage_name: PipelineStageName, pair: str, stage_fn: Callable):
        ...
        try:
            ...
            # Run quality gates
            if stage_name.value in self.quality_gates and output is not None:  # noqa: SIM102
            ...
            if self.config.validation.drift_detection_enabled and isinstance(output, pl.DataFrame):
                drift_report = self.drift_detector.detect_drift(output, stage_name.value, pair, save_as_reference=True)
                stage.metadata["drift"] = drift_report.to_dict()

            # Lineage tracking
            if self.lineage_tracker and output is not None:
                output_rows = None
                ...
                if isinstance(output, pl.DataFrame):
                    ...
                    row_counts = [len(df) for df in output.values() if isinstance(df, pl.DataFrame)]
                    if row_counts:
                        output_rows = sum(row_counts)

                self.lineage_tracker.record_event(
                    LineageEvent(
    ...
    def _stage_labeling(self, pair: str) -> pl.DataFrame:
        ...
        if label_config.method.value in ["rl_reward", "both"]:
            labels = compute_rl_reward_labels_regime(
                ...
                profit_atr_mult=label_config.profit_target_atr,
                stop_atr_mult=label_config.stop_loss_atr,
            )
        ...
        if label_config.method.value in ["triple_barrier", "both"]:
            tb_labels = compute_triple_barrier_labels(
                features=features,
                bars=bars,
                profit_atr_mult=label_config.tb_profit_atr,
                stop_atr_mult=label_config.tb_stop_atr,
                vertical_bars=label_config.tb_horizon_bars,
                pair=pair,
            )

            if label_config.method.value == "both":
                # Combine labels
                labels = labels.merge(tb_labels, on="timestamp_utc", how="outer")
            else:
                labels = tb_labels
    ...
    def _stage_materialization(self, pairs: list[str]) -> dict[str, pl.DataFrame]:
        ...
        return dict.fromkeys(pairs)  # Data already in feature store
        ...
        for stage_name, stage in self.stages.items():
            if stage.output_data is not None and isinstance(stage.output_data, pl.DataFrame):
                drift_report = self.drift_detector.detect_drift(
                    stage.output_data, stage_name.value, save_as_reference=True
                )
    ...
    def get_pipeline_status(self) -> dict:
        ...
        return {
            ...
            "overall": "completed"
            if all(s.status == StageStatus.COMPLETED for s in self.stages.values())
            else "failed"
