...
class DataQualityGates:
    ...
    def _apply_remediation(
        ...
        if action == RemediationAction.DROP_NULLS:
            ...
            df = df.with_columns(
                [pl.col(c).fill_null(strategy="forward").fill_null(0) for c in fill_cols if c in df.columns]
            )
            ...
            fill_cols = params.get("columns", df.select(pl.col(pl.NUMERIC_DTYPES)).columns)
            null_counts = {c: df[c].null_count() for c in fill_cols if c in df.columns}
            df = df.with_columns([pl.col(c).fill_null(0) for c in fill_cols if c in df.columns])
            return df, {"filled_columns": list(null_counts.keys()), "null_counts": null_counts}
            ...
            for col in numeric_cols:
                if col in df.columns:
                    ...
                    exprs.append(pl.col(col).clip(lower_bound=lo, upper_bound=hi).alias(col))
            ...
            if "timestamp_utc" in df.columns:
                ...
                numeric_cols = df.select(pl.col(pl.NUMERIC_DTYPES)).columns
                df = df.with_columns([pl.col(c).interpolate(method=method) for c in numeric_cols])
    ...
    def _check_bid_ask_valid(self, df: pl.DataFrame) -> tuple[bool, str, dict]:
        ...
        invalid = df.filter((pl.col("bid") <= 0) | (pl.col("ask") <= 0) | (pl.col("ask") < pl.col("bid"))).height
    ...
    def _check_spread_positive(self, df: pl.DataFrame) -> tuple[bool, str, dict]:
        ...
        if negative > 0 or zero > 0:
            return (
                False,
                f"Found {negative} negative and {zero} zero spreads",
                {
