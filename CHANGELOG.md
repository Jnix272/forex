...
### Added
- **2026-08-17 — Checkpoint-load report contract fix**: `_strict_load_report()` in `training/model_factory.py` now returns a dict summary with `frac_loaded`, `n_loaded`, `n_target`, `missing`, `unexpected`, and `shape_mismatch`, matching the callers in `training/supervised_loop.py`. This resolves the tuple-vs-dict bug that caused `.get(...)` access errors on the loaded checkpoint report and keeps the report contract stable for pretrain/warm-start flows. Verified with `python -m pyright training/supervised_loop.py training/model_factory.py` → **0 errors**.
...
- **2026-08-16 — Windows quick-mode training pipeline (end-to-end synthetic smoke now passes, Errors: 0)**:
  ...
  - `gpu_datasets.py`: `ZarrStreamDataset.__iter__` now round-robins per-chunk blocks (`self._blocks[wid::n]`) instead of `np.concatenate`-ing every chunk into one block — the giant bool mask in `_decompress_block`'s `nan_to_num` OOM'd (6.14 GiB alloc on fold 1).
  - `hard_example_miner.py`: renamed `OnlineOnlineHardExampleMiner` → `OnlineHardExampleMiner` (double-"Online" typo) so `supervised_loop`'s import succeeds and the miner actually initializes.
  - `direction_control.py`: `_init_multitask_direction_bias` wraps `bias.copy_()` in `torch.no_grad()` — in-place copy on a leaf parameter was raising and silently skipping fold class-prior bias init.
  - `dataset_builder.py`: sequences are now materialized at `_effective_max_seq_len(args)` (curriculum max) instead of `args.seq_len`, and the zarr attrs / manifest `seq_len` written consistently — fixes the `Post-processing cache integrity failed: Manifest mismatch: seq_len 60 != required max 80` that rejected fresh caches built without `--config`. Also changed the `bar_freq` default from `"1min"` to `"5min"` across all build/manifest sites.
  - `post_train.py`: TuneEval dataloader fixed — `ZarrStreamDataset` was called with `seq_len` as the 3rd positional (landing in `shuffle_chunks`) plus nonexistent `targets=`/`augment=` kwargs; now uses `multitask_targets=`. Added missing `_feature_ablation_config` import (was `NameError` in ModelComparison).
