...
def main():
    ...
    parser.add_argument(
        "--pairs", nargs="+", default=MISSING_PAIRS, help="Pairs to download (default: all 7 missing pairs)"
    )
    parser.add_argument("--start-year", type=int, default=2008, help="Start year (default: 2008)")
    parser.add_argument("--end-year", type=int, default=2023, help="End year (default: 2023, 2024 already exists)")
    parser.add_argument("--concurrency", type=int, default=24, help="Concurrent downloads (default: 24)")
    parser.add_argument("--max-retries", type=int, default=8, help="Max retries per hour (default: 8)")
    parser.add_argument(
        "--request-delay", type=float, default=0.05, help="Delay between requests in seconds (default: 0.05)"
    )
    parser.add_argument(
        "--session-only", action="store_true", default=True, help="Only download London+NY session (07-17 UTC)"
    )
    parser.add_argument("--full-day", action="store_true", help="Download full 24h (overrides --session-only)")
    parser.add_argument(
        "--fail-on-missing", action="store_true", default=False, help="Fail if any hours missing after retries"
    )
    parser.add_argument(
        "--max-redownload-passes", type=int, default=3, help="Redownload passes for missing hours (default: 3)"
    )
    parser.add_argument("--resume", action="store_true", default=True, help="Resume from cached data (default: True)")
    parser.add_argument("--no-resume", action="store_false", dest="resume", help="Force re-download all")
    ...
    mgr.duka.concurrency = args.concurrency
    mgr.duka.max_retries = args.max_retries
    mgr.duka.delay = args.request_delay

    try:
        results = mgr.download_dukascopy_year_by_year(
            pairs=args.pairs,
        ...
        for pair, years in results.items():
            ...
            for year, data in years.items():
                ...
                status = "[OK]" if missing == 0 else f"[WARN] {missing}/{requested} missing"
        ...
        print("\n\n[WARN] Interrupted by user. Progress saved in cache.")
        sys.exit(130)
