...
SESSION_HOURS = list(range(7, 18))   # 07:00–17:00 UTC
...
def run():
    ...
    for year in YEARS:
        ...
        for d in candidates:
            ...
            pair_summary = "  ".join(
                f"{p}={'OK' if s['counts'][p] > 0 else '--'}" for p in PAIRS
            )
            print(f"  {year} {date_str}  complete={s['complete']}/9  total={s['total']:>7,}  score={s['score']:>5}  {flag}  |  {pair_summary}")
