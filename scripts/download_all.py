...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--skip-migrate", action="store_true", help="Skip the automatic consolidated-DuckDB migration step")
    ...
    p.add_argument(
        ...
        default=str(defaults["check_missing_scope"]),
        help="Missing-data summary scope",
    )
    p.add_argument(
        "--verify-min-ticks",
        type=int,
        default=int(defaults["verify_min_ticks"]),
    ...
    p.add_argument("--gdelt-min-interval", type=float, default=5.0, help="Minimum seconds between GDELT API requests")
    ...
    # W&B options
    p.add_argument("--wandb-project", type=str, default="forex-scaling-model", help="Weights & Biases project name")
...
def main() -> int:
    ...
    if not args.skip_prices:
        price_cmd = [
            py,
            str(_ROOT / "scripts" / "download_data.py"),
            "--pairs",
            *pairs,
            "--start",
            args.start,
    ...
    if not args.skip_news:
        news_cmd = [
            py,
            str(_ROOT / "scripts" / "download_historical_news.py"),
            "--source",
            "gdelt",
            "--pairs",
            *pairs,
            ...
            "--gdelt-retries",
            str(args.gdelt_retries),
            "--workers",
            str(args.gdelt_workers),
        ]
    ...
    if not args.skip_eco:
        eco_cmd = [
            py,
            str(_ROOT / "scripts" / "scrape_forexfactory.py"),
            "--start",
            args.start,
            "--end",
            args.end,
    ...
    if not args.skip_migrate and not args.skip_prices:
        ...
        migrate_cmd = [
            py,
            str(_ROOT / "scripts" / "migrate_to_duckdb.py"),
            "--pairs",
            *pairs,
        ]
    ...
    print(
        f"  price : yearly={args.yearly} verify={args.verify} fix={args.verify_fix} missing_months={args.check_missing_months}"
    )
    print(
        f"  news  : gdelt_workers={args.gdelt_workers} min_interval={args.gdelt_min_interval}s step_days={args.gdelt_step_days} official_feeds={args.official_feeds}"
    )
