...
def _get(url: str, headers: dict | None = None, timeout: int = 15) -> bytes:
    ...
    req = urllib.request.Request(
        url, headers={**(headers or {}), "User-Agent": "Mozilla/5.0 (forex-scaling-model)"}
    )
...
def check_eodhd() -> None:
    ...
    try:
        ...
        _check("EODHD_API_KEY", ok,
               f"{len(data)} bars returned (EURUSD)" if ok else f"unexpected: {str(data)[:60]}")
...
def check_fred() -> None:
    ...
    try:
        ...
        _check("FRED_API_KEY", ok,
               f"{len(s)} points (US10Y DGS10)" if ok else "empty response")
    except ImportError:
...
def check_discord() -> None:
    ...
    try:
        ...
        _check("DISCORD_WEBHOOK_URL", True,
               f"webhook: {name!r}  guild: {gid}  channel: {ch}")
    except Exception as exc:
...
def check_telegram() -> None:
    ...
    try:
        ...
        data = json.loads(raw)
        bot  = data.get("result", {})
        _check("TELEGRAM_BOT_TOKEN", data.get("ok", False),
...
def check_ollama() -> None:
    ...
    try:
        ...
        if model_loaded:
            ...
            _check("OLLAMA", False,
                   f"running but {model!r} not loaded  ->  ollama pull {model}")
    except Exception:
        _check("OLLAMA", False,
               f"not running at {url}  (start with: ollama serve)")
...
def check_mlflow() -> None:
    ...
    try:
        ...
        _check("MLFLOW", False,
               f"not running at {uri}  (fallback: logs/mlflow_fallback/)")
...
def check_stooq() -> None:
    ...
    try:
        ...
        _check("Stooq", ok,
               f"{len(df):,} daily bars (SPX)" if ok else "unexpected response")
    except Exception as exc:
...
def check_timescale() -> None:
    ...
    try:
        ...
        conn = psycopg2.connect(
            host=host, port=port, dbname=db, user=user, password=pw, connect_timeout=5
        )
        conn.close()
