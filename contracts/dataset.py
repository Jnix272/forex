...
from contracts.base import ContractMetadata, ContractVersion, DataContract, Stage  # noqa: E402
...
class DatasetContract(DataContract):
        ...
        for col in df.columns:
            if df.schema[col].is_numeric():
                ...
                if null_pct > cls.dataset_constraints["max_nan_pct"]:
                    errors.append(
                        f"Column '{col}' has {null_pct:.2%} NaN (max {cls.dataset_constraints['max_nan_pct']:.2%})"
                    )
    ...
    def validate_zarr(cls, zarr_path: str, pair: str | None = None) -> tuple[dict, ContractMetadata]:
        ...
        # Check y_cls values (direction target: -1, 0, 1) — this is the actual
        # classification label used by training; y is continuous reward for Sharpe.
        if y_cls.dtype.kind in ("i", "u"):  # integer dtype
            unique_labels = np.unique(y_cls[:sample_size])
            if len(unique_labels) < 2:
                errors.append(f"y_cls (direction labels) collapsed to single class: {unique_labels}")

        # Check sequence length consistency
        if cls.dataset_constraints["sequence_length_consistency"] and X.ndim == 3:
