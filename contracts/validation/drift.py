    ...
    def detect_drift(
        ...
        if reference is None:
            ...
            return DriftReport(
                ...
                drift_type="none",
                details={"message": "No reference available, baseline created" if save_as_reference else "No reference available"},
            )
        ...
        for col, stats in data_stats.items():
            if col in ref_data_stats:
                ...
                ref_dist = scipy_stats.norm.cdf(bin_edges[1:], ref_mean, ref_std) - scipy_stats.norm.cdf(bin_edges[:-1], ref_mean, ref_std)
