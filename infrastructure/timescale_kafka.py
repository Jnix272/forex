...
class TimescaleDBStore:
    ...
    def write_news(self, df: pd.DataFrame, pair: str = "EURUSD"):
        """Bulk-insert news events into TimescaleDB."""
        if getattr(self, "_is_mock", False):
            records = df.assign(pair=pair).to_dict("records")
            self._mock_news.extend(records)
            return len(records)
    ...
    def read_news(
        ...
        if getattr(self, "_is_mock", False):
            df = pd.DataFrame(self._mock_news)
            if df.empty:
                return pd.DataFrame(columns=["headline", "sentiment_score", "finbert_embedding", "is_high_impact"])
            df = df[df["pair"] == pair].copy()
            if "time" in df.columns:
                df["time"] = pd.to_datetime(df["time"], utc=True)
                return df.set_index("time").sort_index()
        ...
        sql = f"""
            SELECT time, headline, sentiment_score, finbert_embedding, is_high_impact
...
class KafkaTickConsumer:
    ...
    def _flush_buffer(self):
        """Write buffered ticks and news to TimescaleDB in bulk."""
        if self._tick_buffer:
            try:
                ...
                print(f"[Kafka] Flush tick error: {e}")
                self.stats["errors"] += 1

        if self._news_buffer:
            try:
                df = pd.DataFrame(self._news_buffer)
                if "time" in df.columns:
                    df["time"] = pd.to_datetime(df["time"], utc=True)
                    df = df.set_index("time")
                ...
                print(f"[Kafka] Flush news error: {e}")
                self.stats["errors"] += 1
