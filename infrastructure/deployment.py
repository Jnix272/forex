...
Infrastructure & monitoring improvements:
  1. ONNXExporter        - Convert PyTorch models to ONNX for 3-5x faster CPU inference
...
class ONNXExporter:
    ...
    Benefits:
      - 3–5x faster CPU inference vs PyTorch (no Python overhead, optimized kernels)
    ...
    def export(
        ...
        try:
            from inference.onnx_inference import core_onnx_export
    ...
    def _benchmark(self, pytorch_model, onnx_path: str, dummy: "torch.Tensor"):
        ...
        print(f"[ONNX] PyTorch: {pt_ms:.2f}ms | ORT: {ort_ms:.2f}ms | Speedup: {speedup:.1f}x")
...
class ShadowModeManager:
    ...
    def activate(self, candidate_name: str):
        ...
        print(
            f"[Shadow] Started shadow period for '{candidate_name}' | {self.shadow_days} days / {self.shadow_bars} bars"
        )
    ...
    def should_promote(self) -> tuple[bool, dict]:
        ...
        if not self._records or len(self._records) < self.shadow_bars // 2:
            return False, {"reason": "insufficient_data", "bars_recorded": len(self._records)}
        ...
        # Hypothetical P&L: sign of signal x next return
        ...
        from config.settings import GOVERNANCE, MATURITY_LADDER

        min_trades = GOVERNANCE.get("min_trades", 500)
        min_sharpe_promote = MATURITY_LADDER.get("shadow", {}).get("promote_min_sharpe", 1.6)
        ...
        metrics = {
            ...
            "promote": bool(
                agreement >= self.min_agree
                and delta_sharpe >= self.min_sharpe_d
                and cand_sharpe >= min_sharpe_promote
        ...
        if should:
            print(
                f"[Shadow] PROMOTE '{self._candidate_name}' | "
                f"Δ Sharpe: +{delta_sharpe:.3f} | Agreement: {agreement:.1%}"
            )
        else:
            print(f"[Shadow] KEEP production model | Δ Sharpe: {delta_sharpe:+.3f} | Agreement: {agreement:.1%}")
...
class SHAPMonitor:
    ...
    def compute_and_log(
        ...
        """
        print(f"[SHAP] Computing {'gradient' if method == 'gradient' else 'permutation'} importance | Cycle {cycle}")
        ...
        json.dump(record, open(p, "w"), indent=2)  # noqa: SIM115
...
class WalkForwardReporter:
    ...
    def generate_report(
        ...
        window_trades = [t for t in self._trades if pd.Timestamp(t["exit_time"], tz="UTC") >= cutoff]
        win_metrics = self._metrics(window_trades, self._equity[-max(len(window_trades) + 1, 2) :])
        ...
        # Save JSON
        json.dump(report, open(self.report_dir / f"{fname}.json", "w"), indent=2)  # noqa: SIM115
        ...
        print(
            f"[Report] Sharpe: {all_metrics.get('sharpe', 0):.3f} | "
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for i in range(15):  # noqa: B007
            ...
            sm.record(
                pd.Timestamp.now(tz="UTC"), live_sig, cand_sig, 1.085 + rng.normal(0, 0.0003), rng.normal(0, 0.0002)
            )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        m = report["all_time"]
        print(f"\n  Report: {m['n_trades']} trades | Sharpe {m['sharpe']:.3f} | Win {m['win_rate']:.1%}")
    ...
    if TORCH and ONNX:
        ...
        print(f"\n  ONNX: torch={TORCH} onnxruntime={ONNX} (install to enable GPU→CPU export)")
