...
class PretrainHandoffGate:
    ...
    def evaluate_representation_quality(self, embedding_std: float, uniformity: float | None = None) -> bool:
        """
        ...
        if embedding_std < self.std_threshold:
            self.logger.error(
                f"Pretraining Failed: Representation collapsed! (std={embedding_std:.4f} < {self.std_threshold})"
            )
            return False

        if uniformity is not None and uniformity > self.max_uniformity:
            self.logger.error(
                f"Pretraining Failed: Contrastive uniformity too high! (uniformity={uniformity:.4f} > {self.max_uniformity})"
            )
            return False
    ...
    def evaluate_warm_start_shock(self, baseline_val_loss: float, epoch1_val_loss: float) -> bool:
        ...
        if epoch1_val_loss > baseline_val_loss * 1.5:
            self.logger.error(
                f"Warm-Start Shock Detected! Epoch 1 loss ({epoch1_val_loss:.4f}) is vastly worse than baseline ({baseline_val_loss:.4f})."
            )
