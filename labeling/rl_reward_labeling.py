...
def compute_rl_reward_labels(
    ...
    if pl is not None:
        ...
        aligned = (
            features_pl
            .select(["timestamp_utc"] + [c for c in (atr_col, "spread_pips") if c in features_pl.columns])
        ...
        if "ask_close" in aligned.columns and "bid_close" in aligned.columns:
            ...
            spread_vals = (
                ...
                if "spread_pips" in aligned.columns else np.full(len(close), 0.5, dtype=np.float32)
            )
    ...
    valid_market = (
        ...
        & np.isfinite(atr) & (close > 0) & (entry_long > 0) & (entry_short > 0)
        & (atr > 0)
    )
    ...
    result_pl = result_pl.with_columns([
        ...
        pl.when(pl.col("valid_market")).then(pl.col("optimal_side")).otherwise(pl.lit("hold")).alias("optimal_side"),
    ]).drop("valid_market")
    if tail_cut > 0:
    ...
    count_map = dict(zip(counts["label"].to_list(), counts[count_col].to_list()))
    print(f"[RLLabeling] {len(result_pl):,} labels | "
...
def _compute_slippage_multiplier(
    ...
    constant pip assumption. This function produces a per-bar multiplier:
        ...
        mult_t = 1  +  vol_alpha  × max(0, atr_t / median_atr - 1)
                    +  liq_alpha  × max(0, spread_t / median_spread - 1)

    Values: 1.0 = normal, 1.5 = moderately expensive, 2.0+ = rollover/news.
    The result is applied as:  effective_tx_cost = base_tx_cost × mult_t
...
def compute_rl_reward_labels_regime(
    ...
    _spread_vals = np.full(n, 0.5)  # fallback: 0.5 pip spread
    if "spread_pips" in features.columns:
        _spread_vals = features["spread_pips"].fillna(0.5).values
    elif "liquidity_vacuum" in features.columns:
        _spread_vals = features["liquidity_vacuum"].fillna(1.0).values  # proxy

    slippage_mult = _compute_slippage_multiplier(
        atr=atr, spread=_spread_vals,
        vol_window=_vol_win, spread_window=_spr_win,
        vol_alpha=_vol_alpha, liq_alpha=_liq_alpha,
    )
    ...
    if latency_col and latency_col in features.columns:
        latency_ms = features[latency_col].fillna(_lat_base).values.astype(np.float32)
    ...
    for i in range(n - lookahead_bars - delay):
        ...
        if pnl_l > 0:
            ...
            is_spike_win  = (mfe_l > tp_mult * atr[entry_i] / pip_size * 1.5   # MFE well past TP
                             and exit_bar_l <= 2)                          # but exited very fast
        ...
        if pnl_s > 0:
            ...
            is_spike_win  = (mfe_s > tp_mult * atr[entry_i] / pip_size * 1.5
                             and exit_bar_s <= 2)
            if is_large_mae or is_late_exit or is_spike_win:
    ...
    print(f"[RLLabelingRegime] {len(result):,} labels | "
