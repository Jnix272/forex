...
class DatabentoLoader:
    ...
    def load(self, pair: str, start: str | None = None, end: str | None = None) -> pl.DataFrame:
        ...
        lazy_df = lazy_df.with_columns(pl.col(ts_col).cast(pl.Datetime("ns")).dt.replace_time_zone("UTC"))
        ...
        try:
            ...
            first_px = med_df["bid_px_00"][0] if len(med_df) != 0 else 0
            ...
            if pair in self.INVERT_PAIRS:
                ...
                out_lazy = (
                    out_lazy.with_columns(
                        [(1.0 / pl.col("ask")).alias("real_bid"), (1.0 / pl.col("bid")).alias("real_ask")]
                    )
                    .drop(["bid", "ask"])
                    .rename({"real_bid": "bid", "real_ask": "ask"})
                )

            # Add mid and spread
            out_lazy = out_lazy.with_columns(
                [((pl.col("bid") + pl.col("ask")) / 2.0).alias("mid"), (pl.col("ask") - pl.col("bid")).alias("spread")]
            )

            # Add static columns
            out_lazy = out_lazy.with_columns([pl.lit(pair).alias("pair"), pl.lit("databento").alias("source")])
