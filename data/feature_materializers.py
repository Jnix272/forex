...
class BaseMaterializer:
    ...
    def compute(self, spec: FeatureSpec, bars: pl.DataFrame, start: datetime, end: datetime) -> pl.DataFrame | None:
...
class VolatilityMaterializer(BaseMaterializer):
    ...
    def compute(self, spec: FeatureSpec, bars: pl.DataFrame, start: datetime, end: datetime) -> pl.DataFrame | None:
        ...
        if spec.name == "atr_6":
            ...
            return (
                bars.select("timestamp_utc", tr.alias("tr_6"))
                .with_columns(pl.col("tr_6").rolling_mean(window_size=window).alias(spec.name))
                .drop(["tr_6"])
                .drop_nulls()
            )
        ...
        if spec.name == "atr_20":
            ...
            return (
                bars.select("timestamp_utc", tr.alias("tr_20"))
                .with_columns(pl.col("tr_20").rolling_mean(window_size=window).alias(spec.name))
                .drop(["tr_20"])
                .drop_nulls()
            )
        ...
        if spec.name == "rolling_vol_20":
            ...
            return bars.select("timestamp_utc", log_rets.rolling_std(window_size=20).alias(spec.name)).drop_nulls()

        if spec.name == "bollinger_upper_20":
            close = pl.col("close")
            sma = close.rolling_mean(window_size=window)
            std = close.rolling_std(window_size=window)
            return bars.select("timestamp_utc", (sma + 2 * std).alias(spec.name)).drop_nulls()
...
class MicrostructureMaterializer(BaseMaterializer):
    ...
    def compute(self, spec: FeatureSpec, bars: pl.DataFrame, start: datetime, end: datetime) -> pl.DataFrame | None:
        ...
        if spec.name == "hurst_120":
            return (
                bars.select("timestamp_utc", pl.col("close").log().alias("log_close"))
                .with_columns(_hurst_exponent(pl.col("log_close"), 120).alias(spec.name))
                .drop_nulls()
            )

        if spec.name == "fractal_dim_30":
            return bars.select(
                "timestamp_utc", _fractal_dimension(pl.col("log_ret_1"), 30).alias(spec.name)
            ).drop_nulls()
        ...
        if spec.name == "iv_proxy_20":
            ...
            return bars.select(
                "timestamp_utc",
                ((pl.col("high") - pl.col("low")) / pl.col("close")).rolling_mean(window_size=20).alias(spec.name),
            ).drop_nulls()

        if spec.name == "skew_proxy_20":
            log_ret = pl.col("close").log() - pl.col("close").shift(1).log()
            return bars.select("timestamp_utc", log_ret.rolling_skew(window_size=20).alias(spec.name)).drop_nulls()
...
class SessionMaterializer(BaseMaterializer):
    ...
    def compute(self, spec: FeatureSpec, bars: pl.DataFrame, start: datetime, end: datetime) -> pl.DataFrame | None:
        ...
        if spec.name == "hour_sin":
            return bars.with_columns(pl.col("timestamp_utc").dt.hour().cast(pl.Float64).alias("hour_sin")).select(
                "timestamp_utc", (2 * np.pi * pl.col("hour_sin") / 24).sin().alias(spec.name)
            )

        if spec.name == "hour_cos":
            return bars.with_columns(pl.col("timestamp_utc").dt.hour().cast(pl.Float64).alias("hour_cos")).select(
                "timestamp_utc", (2 * np.pi * pl.col("hour_cos") / 24).cos().alias(spec.name)
            )

        if spec.name == "dow_sin":
            return bars.with_columns(pl.col("timestamp_utc").dt.weekday().cast(pl.Float64).alias("dow_sin")).select(
                "timestamp_utc", (2 * np.pi * (pl.col("dow_sin") - 1) / 7).sin().alias(spec.name)
            )

        if spec.name == "dow_cos":
            return bars.with_columns(pl.col("timestamp_utc").dt.weekday().cast(pl.Float64).alias("dow_cos")).select(
                "timestamp_utc", (2 * np.pi * (pl.col("dow_cos") - 1) / 7).cos().alias(spec.name)
            )

        if spec.name == "session_asia":
            hour = pl.col("timestamp_utc").dt.hour()
            return bars.select("timestamp_utc", hour.is_in([23, 0, 1, 2, 3, 4, 5, 6]).cast(pl.Int32).alias(spec.name))

        if spec.name == "session_london":
            hour = pl.col("timestamp_utc").dt.hour()
            return bars.select(
                "timestamp_utc", hour.is_in([7, 8, 9, 10, 11, 12, 13, 14]).cast(pl.Int32).alias(spec.name)
            )

        if spec.name == "session_ny":
            hour = pl.col("timestamp_utc").dt.hour()
            return bars.select(
                "timestamp_utc", hour.is_in([13, 14, 15, 16, 17, 18, 19, 20]).cast(pl.Int32).alias(spec.name)
            )

        if spec.name == "is_monday":
            return bars.select(
                "timestamp_utc", (pl.col("timestamp_utc").dt.weekday() == 1).cast(pl.Int32).alias(spec.name)
            )

        if spec.name == "is_friday":
            return bars.select(
                "timestamp_utc", (pl.col("timestamp_utc").dt.weekday() == 5).cast(pl.Int32).alias(spec.name)
            )
    ...
    def compute(self, spec: FeatureSpec, bars: pl.DataFrame, start: datetime, end: datetime) -> pl.DataFrame | None:
        ...
        if spec.name == "yield_2y10y":
            ...
            if daily.is_empty():
                ...
                built = MacroYieldFeatureBuilder().build(bars if bars is not None else pl.DataFrame())
                if (
                    built is not None
                    ...
                    daily = (
                        ...
                        .with_columns(built["yield_curve_slope"].alias("yield_2y10y"))
                        .drop_nulls()
                    )
                    ...
                    if "US10Y" in yields and "US2Y" in yields:
                        ...
                        slope = (yields["US10Y"].reindex(idx).ffill() - yields["US2Y"].reindex(idx).ffill()).dropna()
            ...
            if out is None or out.is_empty():
                raise RuntimeError("MacroMaterializer: yield_2y10y unavailable from cross-asset/FRED panel")
            return out

        if spec.name == "usd_index":
            if "DXY" not in panel or panel["DXY"] is None:
                raise RuntimeError(
                    "MacroMaterializer: usd_index requires DXY from cross-asset panel (stooq/yahoo). No FX-close alias."
                )
        ...
        if spec.name == "vix_close":
            if "VIX" not in panel or panel["VIX"] is None:
                raise RuntimeError(
                    "MacroMaterializer: vix_close requires VIX from cross-asset panel. No FX-close alias."
                )
...
def _fractal_dimension(return_expr: pl.Expr, window: int = 30) -> pl.Expr:
    ...
    total_range = return_expr.cum_sum().rolling_max(window_size=window) - return_expr.cum_sum().rolling_min(
        window_size=window
    )
...
def materialize_feature(
    ...
    if result is not None and not result.is_empty():
        store._store_materialization(feature_name, result, start, end, MaterializationStrategy.EAGER_BATCH)
