...
def align_asof_available(features_index, observations: pd.DataFrame, *, value_columns=None,
                         event_time_col: str = "event_time", available_time_col: str = "available_time",
                         default_delay: str | pd.Timedelta = "0s") -> pd.DataFrame:
    ...
    merged = pd.merge_asof(left.sort_values("__time"), right, left_on="__time",
                           right_on=available_time_col, direction="backward", allow_exact_matches=True)
