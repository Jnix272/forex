...
def _build_category_flags(df: pl.DataFrame) -> pl.DataFrame | None:
    ...
    if "event_category" in df.columns:
        cat = pl.col("event_category").fill_null("").cast(pl.Utf8).str.strip_chars().str.to_lowercase()
    else:
    ...
    flags = df.select(["timestamp_utc", *onehot_exprs]).group_by("timestamp_utc").max().sort("timestamp_utc")
    return flags
...
@lru_cache(maxsize=4)
def _read_table(path: Path, start: str | None = None, end: str | None = None) -> pl.DataFrame:
    ...
    log_data_load("historical_news_read", str(path), n_rows=len(df), status="ok", t0=_t0, note="csv")
    return df
...
def _filter_relevant(df: pl.DataFrame, start, end, pair: str) -> pl.DataFrame:
    if (len(df) == 0) or "timestamp_utc" not in df.columns:
        return pl.DataFrame()
    out = df.filter((pl.col("timestamp_utc") >= start) & (pl.col("timestamp_utc") <= end))
    if len(out) == 0:
        return out
...
def _load_events(news_file: str | None, calendar_file: str | None, start_ts=None, end_ts=None) -> pl.DataFrame:
    ...
    if calendar_file:
        ...
        log_data_load(
            "eco_calendar",
            str(_DEFAULT_CAL_FILE),
            n_rows=0,
            status="ok" if cal_exists else "skip_missing",
            t0=_t0_cal,
    ...
    for p in paths:
        ...
        if p.suffix.lower() == ".csv":
            ...
            try:
                ...
                log_data_load(
                    "historical_news_path", str(p), n_rows=len(df_slice), status="ok", t0=_t0, note="duckdb slice"
                )
    ...
    *,
    ...
    if "timestamp_utc" in df.columns:
        if "impact" in df.columns:
            ...
            high = imp.is_in(["high", "red", "3", "major"]) | imp.str.contains("high|major|rate|fomc|cpi|nfp")
    ...
    if target_col:
        ...
        if scored_rows.height > 0:
            sentiment = (
                scored_rows.select("timestamp_utc", pl.col(target_col).cast(pl.Float64).alias("sentiment"))
...
def collect_headlines_for_range(
    ...
    *,
    ...
    headlines = raw.select(pl.col("headline").fill_null("").cast(pl.Utf8)).unique().to_series().to_list()
