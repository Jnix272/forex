...
class AlertState:
    ...
    def can_fire(self, cooldown_sec: float, max_per_hour: int) -> bool:
        ...
        # Check rate limit
        return not self.fire_count >= max_per_hour
    ...
    def _register_default_rules(self):
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.CHECK and "nan" in e.tags and not e.payload.get("passed", True)
                ),
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    "oom" in e.payload.get("message", "").lower()
                    or "out of memory" in e.payload.get("message", "").lower()
                ),
                severity=Severity.CRITICAL,
                channels=[AlertChannel.CONSOLE, AlertChannel.DISCORD],
                cooldown_sec=300,
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.CHECK
                    and "grad_norm" in e.payload.get("name", "")
                    and e.payload.get("value", 0) > 100
                ),
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.CHECK and "divergence" in e.name and not e.payload.get("passed", True)
                ),
                severity=Severity.ERROR,
                channels=[AlertChannel.CONSOLE, AlertChannel.DISCORD],
                cooldown_sec=600,
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.METRIC
                    and "val_sharpe" in e.payload.get("name", "")
                    and e.payload.get("value", 1) < 0.5
                ),
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.METRIC
                    and "lr" in e.payload.get("name", "")
                    and e.payload.get("value", 1) < 1e-9
                ),
                severity=Severity.WARNING,
                channels=[AlertChannel.CONSOLE],
                cooldown_sec=3600,
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.CHECK
                    and "drift" in e.name
                    and "critical" in str(e.payload.get("message", "")).lower()
                ),
                severity=Severity.CRITICAL,
                channels=[AlertChannel.CONSOLE, AlertChannel.DISCORD],
                cooldown_sec=1800,
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    e.event_type == EventType.CHECK
                    and "checkpoint_load" in e.name
                    and not e.payload.get("passed", True)
                ),
                severity=Severity.ERROR,
                channels=[AlertChannel.CONSOLE, AlertChannel.DISCORD],
                cooldown_sec=3600,
        ...
        self.add_rule(
            AlertRule(
                ...
                evaluator=lambda e: (
                    "circuit_breaker" in e.payload.get("message", "").lower()
                    or "daily_loss" in e.payload.get("message", "").lower()
                ),
                severity=Severity.CRITICAL,
...
# Global alert engine
_global_alert_engine: AlertRuleEngine | None = None
