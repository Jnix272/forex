...
class EventHandler:
    ...
    def matches(self, event: TrainingEvent) -> bool:
        ...
        if event.severity.value < self.min_severity.value:
            return False
        return not (self.tags and not self.tags.intersection(event.tags))
...
class EventBus:
    ...
    def unregister_handler(self, handler: EventHandler):
        ...
        for _et, handlers in self._handlers.items():
    ...
    async def emit(self, event: TrainingEvent, force: bool = False) -> bool:
        ...
        """
        # Validate payload
        valid, _errors = validate_payload(event.event_type, event.payload)
        if not valid:
        ...
        if not force:
            dedup_key = (
                f"{event.source}:{event.event_type.value}:{event.payload.get('name', event.payload.get('message', ''))}"
            )
    ...
    async def _worker_loop(self):
        ...
        while self._running:
            try:
                ...
                try:
                    _priority, _timestamp, event = await asyncio.wait_for(self._queue.get(), timeout=1.0)
                except TimeoutError:
                    continue
