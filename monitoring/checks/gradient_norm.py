...
def check_gradient_norm(context: CheckContext) -> CheckResult:
    ...
    return CheckResult(
        ...
        message="; ".join(issues) if issues else f"Grad norm OK: total={total_norm:.4f}, max={max_norm:.4f}, min={min_norm:.2e}",
        details={
...
def check_gradient_norm_per_layer(context: CheckContext) -> CheckResult:
    ...
    return CheckResult(
        ...
        status=CheckStatus.PASSED if passed else CheckStatus.FAILED,
        passed=passed,
        message="; ".join(issues) if issues else f"All layer grad norms OK (max: {max(layer_norms.values()) if layer_norms else 0:.4f})",
        details={
