...
class DiscordAlerter:
    ...
    def __init__(
        ...
        if not self._url:
            print(
                "[Discord] No webhook URL set - alerts will only print to console. "
    ...
    def send_circuit_breaker(self, drawdown: float, equity: float, action: str, pair: str = "EURUSD"):
        self.send(
            "circuit_breaker",
            {
                "Pair": pair,
    ...
    def send_training_started(
        self, model: str, run_name: str = "unknown", pairs: list | None = None, data_window: str = "unknown"
    ):
    ...
    def send_production_deploy_completed(
        self, model: str, onnx_path: str = "", schema_path: str = "", fields: dict | None = None
    ):
        from pathlib import Path
    ...
    def send_promotion(self, model: str, sharpe: float, git_hash: str = "", fields: dict | None = None):
    ...
    def send_promotion_gate_passed(self, model: str, sharpe: float, details: dict | None = None):
        self.send(
            "promotion_gate_passed",
            {
                "Model": model,
    ...
    def send_tca_breach(self, cost_pct: float, limit_pct: float, gross_pnl: float):
        self.send(
            "tca_breach",
            {
                "Cost %": f"{cost_pct:.1%}",
    ...
    def _build_embed(self, cfg: dict, fields: dict[str, str], timestamp: str) -> dict:
        embed_fields = [{"name": k, "value": v, "inline": True} for k, v in fields.items()]
