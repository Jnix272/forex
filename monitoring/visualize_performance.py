...
def _plot_loss_overlay(ax: plt.Axes, folds: list) -> None:
    ...
    for fold_data in folds:
        ...
        if tr:
            ax.plot(_epoch_x(tr), tr, color=colour, lw=1.5, label=f"Fold {fi} train")
        if vl:
            ax.plot(_epoch_x(vl), vl, color=colour, lw=1.5, ls="--", alpha=0.7, label=f"Fold {fi} val")
...
def _plot_model_comparison(
    ...
    if not all_models:
        ax.text(0.5, 0.5, "No data", transform=ax.transAxes, ha="center", va="center", color="#8b949e")
        return
...
def _plot_sharpe_heatmap(
    ...
    """Colour-coded epoch x fold Sharpe heatmap."""
    ax.set_title(f"Sharpe Heatmap - {model.upper()} (epoch x fold)", fontsize=10, pad=6)
...
def _plot_summary_table(
    ...
    headers = ["Model", "Run", "Folds", "Epochs (avg)", "Best Sharpe", "Mean Sharpe", "Best Acc", "Ckpt Sharpe"]
    col_w = [0.10, 0.22, 0.06, 0.10, 0.10, 0.10, 0.10, 0.10]

    # Header row
    x_pos = 0.01
    for h, w in zip(headers, col_w, strict=False):
        ax.text(x_pos, 0.92, h, transform=ax.transAxes, fontsize=7.5, color="#58a6ff", fontweight="bold", va="top")
        x_pos += w

    # Divider
    ax.axhline(0.88, color="#30363d", lw=0.8, transform=ax.transAxes, xmin=0.01, xmax=0.99)

    rows_data: list = []
    for _key, entry in sorted(all_logs.items()):
        model = entry["model"]
        run = entry["run"]
        folds = entry["folds"]
        n_fold = len(folds)
    ...
    for ri, (model, run, n_fold, avg_ep, best_sh, mean_sh, best_acc, ckpt_sh) in enumerate(rows_data):
        y = 0.84 - ri * 0.09
        if y < 0.05:
            break
        ...
        x_pos = 0.01
        for vi, (v, w) in enumerate(zip(vals, col_w, strict=False)):
            c = col if vi == 0 else "#c9d1d9"
            if vi in (4, 5) and float(vals[4].replace("-", "-999")) < 0:
                c = "#f78166"
            ax.text(x_pos, y, v, transform=ax.transAxes, fontsize=7.5, color=c, va="top", fontfamily="monospace")
            x_pos += w
...
def build_dashboard(
    ...
    # Title
    next(iter(all_logs.values()))["model"].upper() if all_logs else "-"
    fig.suptitle(
    ...
    for _key, entry in sorted(all_logs.items()):
        ...
        fig.text(
            ...
            fontsize=9,
            color=MODEL_COLOURS.get(model, "#c9d1d9"),
            fontweight="bold",
        ...
        _plot_metric_per_fold(ax_acc, folds, "dir_acc", "Directional Accuracy", "Accuracy", smooth=True)
        row += 1
...
def print_summary(
    ...
    print(f"\n{sep}")
    print(
        f"  {'MODEL':<12} {'RUN':<22} {'FOLDS':>5} {'EP':>4}  "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--log-dir", type=str, default="logs", help="Directory containing *_cv.json training logs (default: logs/)"
    )
    p.add_argument(
        "--checkpoint-dir",
        type=str,
    ...
    p.add_argument("--model", type=str, default=None, help="Filter to one model name  (e.g. haelt)")
    p.add_argument("--run", type=str, default=None, help="Filter to one run prefix  (e.g. haelt_0331)")
    p.add_argument("--out", type=str, default=None, help="Output PNG path  (default: logs/performance_dashboard.png)")
    p.add_argument("--show", action="store_true", help="Open interactive matplotlib window (in addition to saving)")
    p.add_argument("--no-save", action="store_true", help="Do not save PNG - useful with --show on headless servers")
    return p.parse_args()
...
def main() -> None:
    ...
    print(
        f"[Dashboard] Found {len(all_logs)} log file(s), {sum(len(v) for v in ckpt_cfgs.values())} checkpoint config(s)"
    )
