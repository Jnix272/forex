...
Features
--------
  • Human-readable rotating log  -> logs/train_<run>.log   (10 MB x 5 backups)
...
def _gpu_stats() -> dict[str, Any]:
    ...
    try:
        ...
        stats["gpu_temp_c"] = int(pynvml.nvmlDeviceGetTemperature(h, pynvml.NVML_TEMPERATURE_GPU))
    ...
    try:
        ...
        if torch.cuda.is_available() and stats["gpu_mem_mb"] < 0:
            stats["gpu_mem_mb"] = int(torch.cuda.max_memory_allocated() / 1_000_000)
...
class TrainingLogger:
    ...
    def setup(self) -> Path:
        ...
        # ── JSONL event stream ───────────────────────────────────────────────
        self._jlog = open(jsonl_path, "a", encoding="utf-8")  # noqa: SIM115
    ...
    def critical(self, msg: str, exc: Exception | None = None, discord_fields: dict[str, str] | None = None) -> None:
    ...
    def on_epoch_start(self, epoch: int, total_epochs: int, seq_len: int | None = None) -> None:
    ...
    def on_epoch_end(self, epoch: int, metrics: dict[str, Any]) -> None:
        ...
        if self._ep_oom_count:
            msg += f"  ⚠ OOMx{self._ep_oom_count}"
        if self._ep_nan_count:
            msg += f"  ⚠ NaNx{self._ep_nan_count}"
    ...
    def on_batch_progress(self, epoch: int, batch_idx: int, total_batches: int, loss: float, elapsed_s: float) -> None:
        ...
        msg = (
            f"[Epoch {epoch + 1}] Batch {batch_idx}/{total_batches} ({pct:.1f}%) | Loss: {loss:.6f} | {elapsed_s:.1f}s"
        )
        self.info(msg)
        self._write_event(
            "batch_progress",
    ...
    def on_batch_oom(self, batch_idx: int, epoch: int, oom_count: int) -> None:
        ...
        msg = f"[Epoch {epoch + 1}] OOM at batch {batch_idx} (total this epoch: {self._ep_oom_count})"
        self.warning(msg)
        self._write_event(
            "oom",
            {
    ...
    def on_batch_nan(self, batch_idx: int, epoch: int, nan_count: int) -> None:
        ...
        msg = f"[Epoch {epoch + 1}] NaN loss at batch {batch_idx} (total this epoch: {self._ep_nan_count})"
        self.warning(msg)
        self._write_event(
            "nan_loss",
            {
    ...
    def on_nan_gradient(self, epoch: int, param_names: list[str]) -> None:
        ...
        msg = f"[Epoch {epoch + 1}] NaN gradients in: {', '.join(param_names[:5])}" + (
    ...
    def on_grad_norm(self, epoch: int, batch_idx: int, grad_norm: float, threshold: float = 50.0) -> None:
        ...
        if grad_norm > threshold * 2.0:
            self.warning(
                f"[Epoch {epoch + 1}] High grad norm ({grad_norm:.2f}) at batch {batch_idx} (threshold {threshold})"
            )
    ...
    def on_thermal_throttle(self, temp_c: int, limit_c: int) -> None:
        ...
        msg = f"[GPU] Thermal throttle - {temp_c}°C ≥ limit {limit_c}°C. Pausing training."
        self.warning(msg)
    ...
    def on_promotion_decision(
        ...
        self.info(f"[PromotionGate] {model_name}: {verdict} | {metric_name}={metric_value} | {gate_summary}")
    ...
    def on_rl_complete(self, stats: dict[str, Any]) -> None:
        ...
        self._safe_log(
            logging.INFO,
            "[RL] stats - " + ", ".join(f"{k.split('/')[-1]}={v}" for k, v in stats.items() if k.startswith("rl/")),
        )
    ...
    def _write_event(self, event: str, data: dict[str, Any]) -> None:
        if self._jlog is None:
            return
        record = {"ts": _utcnow(), "event": event, "run": self.run_name, "model": self.model_name, **data}
        try:
    ...
    def _on_watchdog_timeout(self, gap_s: float) -> None:
        msg = f"[Watchdog] No training progress for {gap_s / 60:.1f} min - training may be frozen or waiting on I/O."
        self.warning(msg)
        self._discord_send(
            "drift_detected",
            {
