...
Infrastructure + Backtesting upgrades (7 items):
  1. ONNXExporter            - PyTorch -> ONNX -> 3-5x faster CPU inference
  ...
  5. MonteCarloBacktest      - Randomise trade order 1000x for CI on Sharpe
...
class ONNXExporter:
    ...
    def _quantize_int8(self, onnx_path: str) -> str:
        """Quantise ONNX model to INT8 for further 2x speedup."""
        try:
...
class ShadowModeDeployer:
    ...
    def step(
        ...
        actual_return: float,  # Bar return (direction x |pnl|)
    ):
...
class SHAPFeatureTracker:
    ...
    For 333k samples x 56 features, we use SHAP's DeepExplainer with
    ...
    def compute(
        ...
        if self._prev_top5 is not None:
            ...
            if overlap < (1 - self.alert_thresh):
                print(f"[SHAP] ⚠ Feature importance shift detected! Top-5 changed {(1 - overlap) * 100:.0f}%")
    ...
    def log_to_wandb(self, importance_df: pd.DataFrame, run: Any = None):
        ...
        try:
            ...
            table = wandb.Table(data=list(zip(top20["feature"], top20["mean_shap"], strict=False)), columns=["feature", "mean_shap"])
            wb_run.log(
                {"feature_importance": wandb.plot.bar(table, "feature", "mean_shap", title="SHAP Feature Importance")}
            )
        except Exception as e:
...
class SlippageCalibrator:
    ...
    Model: slippage_pips = α x (size / ADV)^β
    ...
    def fit(
        ...
        # Log-linear regression: log(slip) = log(α) + β x log(size_pct)  # noqa: RUF003
        mask = (size_pct > 0) & (slip > 0)
        if mask.sum() < 5:
            print("[Slippage] Insufficient data - using defaults α=0.1, β=0.5")  # noqa: RUF001
            self.alpha = 0.1
            self.beta = 0.5
        ...
        self._fitted = True
        print(f"[Slippage] Calibrated: α={self.alpha:.4f} β={self.beta:.4f} R²={r2:.3f}")  # noqa: RUF001
...
class LockboxEvaluator:
    ...
    def evaluate(
        ...
        if self._evaluated:
            ...
            print(
                f"[Lockbox] WARNING: Already evaluated on {prev.get('date')}! "
                f"Cannot evaluate again. Previous result: {prev}"
            )
            return prev

        print("\n[Lockbox] *** FINAL EVALUATION *** Breaking the seal...")
        print(f"[Lockbox] Model: {model_name} | Samples: {len(X_lockbox):,}")
...
if __name__ == "__main__":
    ...
    print(f"  Monte Carlo Sharpe CI ({result['confidence']:.0%}): {result['sharpe_ci']}")
    ...
    for _i in range(60):
        sm.step(rng.integers(0, 3), rng.integers(0, 3), rng.normal(0, 0.001))
    promote, diag = sm.should_promote()
    print(f"  Shadow result: {diag['status']} | ΔSharpe={diag['sharpe_improvement']}")

    # Lockbox
    lb = LockboxEvaluator(lock_file=str(Path(tempfile.gettempdir()) / "lockbox_test.json"))
    df = pd.DataFrame(np.random.randn(1000, 5), index=pd.date_range("2023-01-01", periods=1000, freq="D", tz="UTC"))
