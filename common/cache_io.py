"""Neutral Zarr / NPY cache-path helpers shared by ``data/`` and ``training/``.

Moved verbatim from ``training/gpu_cache_io.py`` (refactor R1) so that
``data/feature_cache.py`` no longer needs to import from ``training/`` --
breaking the ``data -> training -> data`` import cycle.
``training/gpu_cache_io`` remains as a backward-compat re-export shim.
"""
