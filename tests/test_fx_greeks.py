...
def test_atm_put_delta_negative():
    ...
    # Black-76 put-call delta parity: Δ_call − Δ_put = exp(−r_f·t)
    import math
...
def _leg(pair="EURUSD", spot=1.10, strike=1.10, quantity=1.0, call=True):
    return {"pair": pair, "spot": spot, "strike": strike,
            "tenor_years": 0.5, "vol": 0.10, "quantity": quantity, "call": call}
...
def test_portfolio_aggregate_sums_greeks():
    ...
    assert agg["delta"] == pytest.approx(
        compute_greeks("EURUSD", 1.10, 1.10, 0.5, 0.10).delta
        + compute_greeks("GBPUSD", 1.30, 1.30, 0.5, 0.10).delta,
        abs=1e-4,
    )
...
def test_net_delta_by_currency():
    ...
    assert nd["EUR"] == pytest.approx(
        compute_greeks("EURUSD", 1.10, 1.10, 0.5, 0.10).delta
        + compute_greeks("EURGBP", 0.85, 0.85, 0.5, 0.10).delta,
        abs=1e-4,
    )
