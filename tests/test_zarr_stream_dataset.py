...
def _build_cache(tmp_path: Path, *, n_rows: int = 64, seq_len: int = 4,
                 n_features: int = 3, chunk_rows: int = 8,
                 with_pq: bool = True, with_y_cls: bool = True):
    ...
    if with_pq:
        _zarr_create(z, "pq", shape=pq.shape, chunks=(chunk_rows,),
                     dtype=ZARR_LABEL_DTYPE, compressor=Comp)
        z["pq"][:] = pq
    return str(p), X.astype(np.float32), y, (
        y_cls if with_y_cls else None), (pq if with_pq else None)
...
def test_basic_iteration_yields_all_rows(tmp_path: Path):
    cache, X, y, y_cls, pq = _build_cache(tmp_path)
...
def test_block_compression_alignment_uses_chunk_size(tmp_path: Path):
    ...
    cache, X, y, _, _ = _build_cache(tmp_path, n_rows=64, chunk_rows=8)
...
def test_multitask_uses_published_pq(tmp_path: Path):
    cache, X, y, y_cls, pq = _build_cache(tmp_path, with_pq=True, with_y_cls=True)
    ds = ZarrStreamDataset(cache, np.arange(len(y)), shuffle_chunks=False,
                           multitask_targets=True, return_indices=False)
    x_t, y_t, yc_t, pq_t = next(iter(ds))
...
def test_multitask_pq_fallback_is_unity_when_pq_missing(tmp_path: Path):
    ...
    cache, X, y, y_cls, pq = _build_cache(tmp_path, with_pq=False, with_y_cls=True)
    ds = ZarrStreamDataset(cache, np.arange(len(y)), shuffle_chunks=False,
...
def test_shuffle_buffer_makes_churn_across_epochs(tmp_path: Path):
    ...
    *only when* the seed differs. With a fixed seed, the order is reproducible."""
    cache, X, y, _, _ = _build_cache(tmp_path, n_rows=128, chunk_rows=8)
    ds = ZarrStreamDataset(cache, np.arange(128), shuffle_chunks=True,
                           shuffle_buffer_size=64, shuffle_seed=42)
    ...
    ds2 = ZarrStreamDataset(cache, np.arange(128), shuffle_chunks=True,
                           shuffle_buffer_size=64, shuffle_seed=42, return_indices=True)
    order1 = [idx.item() for *_, idx in ds2]
    ds3 = ZarrStreamDataset(cache, np.arange(128), shuffle_chunks=True,
                           shuffle_buffer_size=64, shuffle_seed=42, return_indices=True)
...
def test_shuffle_buffer_zero_is_within_block_only(tmp_path: Path):
    ...
    cache, X, y, _, _ = _build_cache(tmp_path, n_rows=128, chunk_rows=8)
    ds = ZarrStreamDataset(cache, np.arange(128), shuffle_chunks=True,
                           shuffle_buffer_size=0, shuffle_seed=7)
...
def test_per_worker_rng_reproducible_under_same_seed(tmp_path: Path):
    ...
    cache, X, y, _, _ = _build_cache(tmp_path, n_rows=64, chunk_rows=8)
    ds_a = ZarrStreamDataset(cache, np.arange(64), shuffle_chunks=False,
                             shuffle_seed=7)
    ds_b = ZarrStreamDataset(cache, np.arange(64), shuffle_chunks=False,
                             shuffle_seed=7)
...
def test_return_indices_3tuple_when_multitask_off(tmp_path: Path):
    cache, X, y, _, _ = _build_cache(tmp_path, with_pq=False, with_y_cls=False)
    ds = ZarrStreamDataset(cache, np.arange(len(y)), shuffle_chunks=False,
                           return_indices=True, multitask_targets=False)
    sample = next(iter(ds))
    assert len(sample) == 3
...
def test_return_indices_5tuple_when_multitask_on(tmp_path: Path):
    cache, X, y, y_cls, pq = _build_cache(tmp_path, with_pq=True, with_y_cls=True)
    ds = ZarrStreamDataset(cache, np.arange(len(y)), shuffle_chunks=False,
                           return_indices=True, multitask_targets=True)
    sample = next(iter(ds))
    assert len(sample) == 5
...
def test_multitask_4tuple_without_indices(tmp_path: Path):
    cache, X, y, y_cls, pq = _build_cache(tmp_path, with_pq=True, with_y_cls=True)
    ds = ZarrStreamDataset(cache, np.arange(len(y)), shuffle_chunks=False,
                           return_indices=False, multitask_targets=True)
    sample = next(iter(ds))
    assert len(sample) == 4
...
def test_dataloader_iteration_single_process(tmp_path: Path):
    cache, X, y, y_cls, pq = _build_cache(tmp_path, n_rows=32, chunk_rows=8,
                                         with_pq=True, with_y_cls=True)
    ds = ZarrStreamDataset(cache, np.arange(32), shuffle_chunks=True,
                           shuffle_buffer_size=32, shuffle_seed=42,
                           multitask_targets=True, return_indices=True)
