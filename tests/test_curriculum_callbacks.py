...
class TestBaseCurriculum:
    ...
    def test_loss_weighting(self):
        ...
        config = CurriculumConfig(
            pace_function="linear", total_epochs=10, min_fraction=0.1, use_loss_weighting=True, loss_temperature=1.0
        )
        curriculum = BaseCurriculum(config, difficulty)
...
class TestCustomCurriculumAdapter:
    ...
    def test_adapter_difficulty_mode(self):
        ...
        difficulty = np.linspace(0, 1, 100)
        adapter = CustomCurriculumAdapter(difficulty, mode="difficulty", pace_function="linear", advance_rate=0.1)
    ...
    def test_adapter_state_dict(self):
        ...
        # Load state
        adapter2 = CustomCurriculumAdapter(difficulty, mode="difficulty", pace_function="linear", advance_rate=0.1)
...
class TestFactoryFunction:
    ...
    def test_create_pytorch_lightning(self):
        ...
        callback = create_curriculum_callback(
            framework="pytorch_lightning", difficulty_scores=difficulty, pace_function="linear", total_epochs=10
        )
    ...
    def test_create_composer(self):
        ...
        difficulty = np.linspace(0, 1, 100)
        callback = create_curriculum_callback(
            framework="composer", difficulty_scores=difficulty, pace_function="linear", total_epochs=10
        )
    ...
    def test_create_invalid_framework(self):
        ...
        with pytest.raises(ValueError):
            create_curriculum_callback(framework="invalid", difficulty_scores=difficulty)
...
class TestIntegrationHelpers:
    ...
    def test_make_curriculum_datamodule(self):
        ...
        from training.curriculum_callbacks import PLCurriculumCallback, make_curriculum_datamodule
