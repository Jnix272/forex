...
(``positions × pip_value`` ≈ 10) with a covariance matrix of price-fraction
...
After the fix: weights are dollar-notional (£100k × lots) and returns are
price-fraction scale; units are consistent ($/lot × lots → $; cov in
...
Sanity-check math (closed-form, no simulation):
  ...
  Var(P&L) = N² × σ_ret² = $10¹⁰ × 9e-8 = $900
  σ_PnL = $30
  z_99% = 2.326
  VaR_1day = z × σ_PnL ≈ $69.78
"""
...
def _build_with_returns(returns_by_pair: dict, n_obs: int = 100,
...
def test_var_usd_is_in_dollars_not_cents():
    ...
    # The fix should produce var_usd ≈ $69 (z=2.326 × $100k × 3e-4 ≈ $69.78)
    assert var_usd > 1.0, (
        f"var_usd={var_usd} is too small — likely still using pip-dollar "
        "× price-fraction-cov (old bug) or notional × pips-cov (off-by-pip_size)"
    )
    assert var_usd < 1000.0, (
        f"var_usd={var_usd} is too large — likely double-scaled"
    )
...
def test_var_pct_is_a_nontrivial_fraction_of_equity():
    ...
    assert var_pct > 1e-4, (
        f"var_pct={var_pct} is too small to ever trip a 2% daily VaR gate"
    )
...
def test_var_usd_scales_with_notional():
    ...
    assert v2["var_usd"] > 1.9 * v1["var_usd"], (
        "Doubling lots should approximately double VaR"
    )
    assert v2["var_usd"] < 2.1 * v1["var_usd"], (
        "VaR scaling with lot count should be near linear (single pair)"
    )


def test_var_usd_scales_with_return_volatility():
    """10× the return std → VaR should scale 10× (linear in σ, since z is const)."""
    ...
    assert v_high["var_usd"] > 9.5 * v_low["var_usd"], (
        f"10Ï returns should produce ~10Ã VaR (got {v_high['var_usd']} vs "
        f"{v_low['var_usd']})"
    )
...
def test_cvar_exceeds_var_for_known_distribution():
    ...
    construction. The code uses `cvar = σ × φ(z) / (1 - conf)` (Tail Conditional
...
def test_pip_scaled_returns_are_normalised_to_price_fraction():
    ...
    assert var_usd < 200.0, (
        f"After pip-size normalization, var_usd={var_usd} should be small; "
        "if large, the normalizer is broken"
    )
...
def test_set_notional_overrides_default():
    ...
    # $100 notional × ~1e-3 std × 2.326 z ≈ $0.23
    assert 0.05 < result["var_usd"] < 1.5, (
        f"var_usd={result['var_usd']} out of expected range for overridden "
        "notional"
    )
...
# ---------------------------------------------------------------------------
# Closed-form number verification (z·σ·notional)
# ---------------------------------------------------------------------------

def test_var_usd_matches_closed_form_calculation():
    """For a single pair, VaR = z × σ × N (no correlation complexity).
    ...
    # So to get std=3e-4, set amplitude = 3e-4 × √2
    ...
    expected_var_usd = 2.326 * std_actual * 100_000 * 1.0  # z × σ × N × lots
