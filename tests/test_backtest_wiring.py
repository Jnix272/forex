...
def test_meta_labeler_mask_disabled_when_args_off(window_data):
    ...
    mask, _meta = _build_meta_labeler_mask(args, bars, feats, cls, seq_len)
...
def test_meta_labeler_mask_small_window_returns_none():
    ...
    feats = pd.DataFrame(
        {"atr_6": rng.uniform(0.0001, 0.0005, n), "spread_pips": 1.0, "rsi_14": 50.0, "adx_14": 20.0, "ret_5": 0.0},
        index=idx,
    )
    cls = np.zeros(n - 20, dtype=int)
    args = _make_args(seq_len=20)
    _mask, meta = _build_meta_labeler_mask(args, bars, feats, cls, 20)
...
def test_advanced_execution_overlay_advanced(window_data):
    ...
    sigs = [{"timestamp": bars.index[100], "action": 1}, {"timestamp": bars.index[150], "action": 2}]
    eff, meta = _advanced_execution_overlay(args, bars, sigs)
    assert eff >= 0.7
