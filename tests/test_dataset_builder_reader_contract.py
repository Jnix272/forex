...
def _build_cache(
    ...
    *,
    ...
    if with_pq:
        pq = pq_array if pq_array is not None else rs.uniform(0.0, 1.0, n_rows).astype(np.float32)
    else:
        pq = None
    if with_y_cls:
        y_cls = y_cls_array if y_cls_array is not None else rs.choice([-1.0, 0.0, 1.0], n_rows).astype(np.float32)
    else:
        y_cls = None
    if with_diff:
        diff = diff_array if diff_array is not None else rs.choice([0, 1, 2], n_rows).astype(np.uint8)
    else:
        diff = None
    ...
    if pq is not None:
        ...
        z["pq"][:] = pq
    if diff is not None:
        _zarr_create(z, "diff", shape=diff.shape, chunks=(chunk_rows,), dtype="uint8", compressor=Comp)
        z["diff"][:] = diff
...
def test_clean_cache_passes(tmp_path: Path):
    cache = _build_cache(
        tmp_path, n_rows=200, chunk_rows=64, with_pq=True, with_y_cls=True, with_diff=True, with_scaler=True
    )
    ok, problems = _validate_dataset_builder_reader_contract(
        cache,
        _args(multitask=True, label_method="rl_reward"),
    )
    assert ok, problems
    assert problems == []
...
def test_multitask_rl_reward_missing_pq_warns(tmp_path: Path):
    ...
    _ok, problems = _validate_dataset_builder_reader_contract(
        cache,
        _args(multitask=True, label_method="rl_reward"),
    )
...
def test_y_cls_outside_class_set_is_hard_error(tmp_path: Path):
    ...
    n = 200
    bad_y_cls = np.array([0.0, 0.3, 1.5, -0.2, 2.7] * (n // 5) + [0.0] * (n % 5), dtype=np.float32)
...
def test_scaler_shape_mismatch_is_hard_error(tmp_path: Path):
    ...
    cache = _build_cache(tmp_path, n_features=3, with_scaler=True, scaler_scale=np.ones(7, dtype=np.float64))  # 7 != 3
    ok, problems = _validate_dataset_builder_reader_contract(
        cache,
        _args(),
    )
...
def test_scaler_shape_match_passes(tmp_path: Path):
    cache = _build_cache(tmp_path, n_features=3, with_scaler=True, scaler_scale=np.ones(3, dtype=np.float64))
    ok, problems = _validate_dataset_builder_reader_contract(
        cache,
        _args(),
    )
...
def test_diff_wrong_dtype_is_error(tmp_path: Path):
    ...
    _zarr_create(z, "diff", shape=bad.shape, chunks=(64,), dtype="int32", compressor=None)
    z["diff"][:] = bad
    ok, problems = _validate_dataset_builder_reader_contract(
        cache,
        _args(),
