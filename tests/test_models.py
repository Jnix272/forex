...
class TestForexTradingEnv:
    ...
    def test_buy_then_sell_closes(self):
        ...
        env.reset()
        env.step(ScalingAction.OPEN_LONG.value)
        assert env.position > 0
        env.step(ScalingAction.CLOSE_ALL.value)
...
class TestTrainMetaLearner:
    ...
    def test_meta_training_checkpoints_each_epoch(self, tmp_path):
        """Meta training should leave a usable checkpoint before final return."""
        F = 48
        bases = [TinySeqModel(F), TinySeqModel(F)]
        ens = EnsembleMetaLearner(bases, context_dim=16, hidden=32)
        loader = self._make_loader(F=F, n=64)
        ckpt = tmp_path / "ensemble_meta_best.pt"
        ...
        history = train_meta_learner(
            ...
            checkpoint_meta={"base_names": ["a", "b"], "n_features": F},
        )

        latest = tmp_path / "ensemble_meta_best_latest.pt"
        assert ckpt.exists()
        assert latest.exists()
        assert (tmp_path / "ensemble_meta_best.pt.json").exists()
        assert (tmp_path / "ensemble_meta_best_latest.pt.json").exists()
...
class TestMultiPairWrapper:
    ...
    @staticmethod
    def _n_interaction(n_pairs: int) -> int:
        """Matches MultiPairWrapper: 3*n_cross + n_pairs + 2 interaction features."""
        n_cross = n_pairs * (n_pairs - 1) // 2
        return 3 * n_cross + n_pairs + 2
...
class TestMultiPairHelpers:
    ...
    def test_multipair_chunk_reindexes_by_common_timestamps(self):
        ...
        def chunk(values, times):
            n = len(values)
            X = np.asarray(values, dtype=np.float32).reshape(n, 1, 1)
            X = np.repeat(X, T, axis=1)
            y = np.asarray(values, dtype=np.float32)
            diff = np.zeros(n, np.uint8)
            pq = np.ones(n, np.float32)
            y_cls = np.zeros(n, np.float32)
            close = y + 100.0
            atr = np.full(n, 0.001, np.float32)
            spread = np.full(n, 0.0001, np.float32)
            return X, y, diff, pq, y_cls, close, atr, spread, F, np.asarray(times)
