...
def sample_bars() -> pl.DataFrame:
    ...
    n = 300
    ts = [datetime(2024, 1, 2, 10, 0, tzinfo=UTC) + timedelta(minutes=i) for i in range(n)]
...
class TestSentimentTiers:
    def test_adds_sentiment_columns_when_present(self):
        ...
        ts = [datetime(2024, 1, 2, 10, i, tzinfo=UTC) for i in range(5)]
        df = pl.DataFrame(
            {
                "timestamp_utc": ts,
...
class TestFeatureEngineerBuild:
    ...
    def test_build_has_news_cat_columns(self, sample_bars):
        ...
        fe = FeatureEngineer()
        out = fe.build(sample_bars)
        cats = ["cat_central_bank", "cat_inflation", "cat_labor", "cat_growth", "cat_geopolitical", "cat_commentary"]
        for col in cats:
