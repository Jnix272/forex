...
def test_visualizer_command_execution(tmp_path, model):
    ...
    cmd = [PYTHON_EXE, str(VISUALIZER_PATH), "--model", model, "--output", str(output_file)]

    result = subprocess.run(cmd, capture_output=True, text=True)
    assert result.returncode == 0, (
        f"visualize_backtest.py failed for model {model}.\nSTDOUT: {result.stdout}\nSTDERR: {result.stderr}"
    )
...
def test_visualizer_invalid_model(tmp_path):
    ...
    cmd = [PYTHON_EXE, str(VISUALIZER_PATH), "--model", "invalid_model_type", "--output", str(output_file)]
...
def test_visualizer_missing_parameters():
    ...
    cmd = [PYTHON_EXE, str(VISUALIZER_PATH)]
    result = subprocess.run(cmd, capture_output=True, text=True)
...
def test_visualizer_real_world_pipeline(tmp_path):
    ...
    cmd = [PYTHON_EXE, str(VISUALIZER_PATH), "--model", "rl", "--output", str(output_file)]
