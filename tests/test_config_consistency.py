...
def test_every_python_profile_is_supported():
    for name in MODELS:
        assert name in SUPPORTED_SUPERVISED, f"MODELS contains '{name}' but it's not in SUPPORTED_SUPERVISED"
...
def test_yaml_model_names_match_filename(yaml_configs):
    for stem, cfg in yaml_configs.items():
        ...
        assert model_name == stem, f"{stem}.yaml declares model.name='{model_name}', expected '{stem}'"
...
def test_no_python_profiles_use_legacy_key_names():
    ...
    for name, profile in MODELS.items():
        for bad_key, suggestion in legacy_keys.items():
            assert bad_key not in profile, f"MODELS['{name}'] has legacy key '{bad_key}' - {suggestion}"


def test_consistent_num_layers_key():
    for _name, profile in MODELS.items():
...
def test_warmup_less_than_half_epochs():
    ...
    assert warmup < epochs * 0.5, f"Warmup ({warmup}) >= 50% of epochs ({epochs})"
...
def test_lr_comes_from_yaml_not_training_dict():
    ...
    assert "lr" not in TRAINING, "LR should not be in TRAINING dict - it comes from YAML/CLI config"
...
def test_strategy_profiles_have_required_keys():
    ...
    for name, profile in STRATEGY_PROFILES.items():
        for key in required:
            assert key in profile, f"STRATEGY_PROFILES['{name}'] missing required key '{key}'"


def test_strategy_seq_len_within_curriculum():
    for name, profile in STRATEGY_PROFILES.items():
        assert profile["seq_len"] <= 120, f"Strategy '{name}' seq_len={profile['seq_len']} exceeds max curriculum (120)"
