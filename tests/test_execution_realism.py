...
def _market(n=12):
    ...
    close[2:] = 1.1010
    bars = pd.DataFrame({"open": close, "high": close + .0001, "low": close - .0001,
                         "close": close, "bid_close": close - .00005,
...
def test_labels_enter_after_execution_delay_and_emit_action_utilities():
    bars, feats = _market()
    out = realistic_utility_labels(bars, feats, lookahead_bars=3, execution_delay_bars=2,
...
def test_ambiguous_bar_is_resolved_conservatively_stop_first():
    ...
    out = realistic_utility_labels(bars, feats, lookahead_bars=2, execution_delay_bars=1,
                                   fill_model=EmpiricalFillModel(randomize=False, base_slippage_pips=0))
    assert out.iloc[0].utility_long < 0
...
def test_rolling_splits_keep_purge_and_lockbox_untouched():
    splits = list(rolling_retraining_splits(100, retrain_every=10, validation_size=10,
                                            min_train_size=40, purge_bars=5, lockbox_size=20))
    assert splits
