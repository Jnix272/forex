...
def _resolve_activation(name: str) -> "type[nn.Module]":
    ...
    if cls_name is None:
        ...
        raise ValueError(
            f"Unknown activation '{name}'. Supported: {sorted(_ACTIVATION_MAP)}. "
            "Or pass the exact torch.nn class name."
        )
...
class RLConfig:
    ...
    # Custom RL specific
    custom_reward_weights: Optional[dict] = None
...
class BaseRLAdapter(ABC):
    ...
    @abstractmethod
    def load(self, path: str, env: Optional[Any] = None) -> None:
...
class CleanRLAdapter(BaseRLAdapter):
    ...
    def _import_cleanrl(self, algorithm: str):
        ...
        try:
            ...
            raise ImportError(
                f"CleanRL not installed. Install with: "
                f"pip install git+https://github.com/vwxyzjn/cleanrl.git"
            )
    ...
    def train(
        ...
        **kwargs
        ...
        obs_size = env.obs_size if hasattr(env, "obs_size") else env.observation_space.shape[0]
        n_actions = env.n_actions if hasattr(env, "n_actions") else env.action_space.n
        
        # Create agent
        # Note: CleanRL agents expect specific arguments
        device = self.config.device if self.config.device != "auto" else ("cuda" if torch.cuda.is_available() else "cpu")
...
class GymEnvWrapper:
    ...
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            ...
            shape=(env.obs_size,), dtype=np.float32
        )
    ...
    **kwargs
    ...
    if framework in ("cleanrl",):
        ...
        raise ValueError(f"Unknown framework: {framework}. "
                         f"Choose from: cleanrl, sb3, stable_baselines3, custom")
