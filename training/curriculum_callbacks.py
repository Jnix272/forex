...
class BaseCurriculum:
    ...
    def _pace(self, epoch: int) -> float:
        ...
        if self.config.pace_function == "linear":
            ...
            warnings.warn(f"Unknown pace function: {self.config.pace_function}, using linear", stacklevel=2)
            return min_frac + span * e

    def update(self, epoch: int, losses: np.ndarray | None = None) -> dict[str, Any]:
...
class PLCurriculumCallback(pl.Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        if self.verbose:
            included = mask.sum()
            print(
                f"[PLCurriculum] Epoch {epoch}: pace={info['pace']:.3f}, "
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if self.curriculum.config.use_loss_weighting and isinstance(outputs, dict):  # noqa: SIM102
...
class ComposerCurriculumCallback:
    ...
    def apply(self, event, state, logger):
        ...
        if event == "EPOCH_START":
            ...
            losses = None
            if self.curriculum.config.use_loss_weighting:  # noqa: SIM102
            ...
            if self.verbose:
                ...
                included = mask.sum()
                print(
                    f"[ComposerCurriculum] Epoch {epoch}: pace={info['pace']:.3f}, "
                    f"included={included}/{len(mask)} ({info['inclusion_rate']:.1%})"
                )

        elif event == "BATCH_END" and self.curriculum.config.use_loss_weighting:
            # Collect per-sample losses
            if hasattr(state, "outputs") and isinstance(state.outputs, dict):  # noqa: SIM102
...
def create_curriculum_callback(
    ...
    mode: str = "difficulty",
    **kwargs,
) -> PLCurriculumCallback | ComposerCurriculumCallback | CustomCurriculumAdapter:
    """
...
class CustomCurriculumAdapter:
    ...
    def _init_curriculum(self):
        ...
        if self.mode == "difficulty":
            ...
            from training.curriculum import SelfPacedConfig, SelfPacedLearning
            ...
            diff_config = DifficultyCurriculumConfig(
                **{
                    k: v
    ...
    def get_inclusion_mask(self) -> np.ndarray:
        ...
        if self.mode == "difficulty":
            ...
            diff_mask = self._diff_curriculum.get_inclusion_mask()
            sp_weights = self._curriculum.get_weights(getattr(self._curriculum, "current_epoch", 0), None)
...
def make_curriculum_datamodule(
    base_datamodule,
    curriculum_callback: PLCurriculumCallback | CustomCurriculumAdapter,
) -> type:
    """
