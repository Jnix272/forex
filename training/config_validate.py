...
def _curriculum_max_epoch(curriculum: dict | None, key: str) -> int:
    ...
    epochs = [
        int(e.get("epoch_start", 0)) for e in schedule if isinstance(e, dict) and e.get("epoch_start") is not None
    ]
...
def resolve_models_to_train(
    ...
    *,
    ...
    if getattr(args, "all_models", False):
        requested = [m.strip().lower() for m in str(getattr(args, "models", "") or "").split(",") if m.strip()]
        if requested:
            unknown = [m for m in requested if m not in SUPPORTED_SUPERVISED]
            if unknown:
                raise ValueError(f"Unknown model(s) {unknown}; expected one of {sorted(SUPPORTED_SUPERVISED)}")
            models = requested
        else:
    ...
    if apply_resume_filter and getattr(args, "all_models", False) and getattr(args, "resume", False):  # noqa: SIM102
...
def collect_config_issues(
    ...
    if warmup >= epochs:
        errors.append(f"lr_warmup_epochs={warmup} >= epochs={epochs} - model never trains at target LR.")
    ...
    if model_name not in SUPPORTED_SUPERVISED:
        errors.append(f"Unknown model '{model_name}'. Valid: {sorted(SUPPORTED_SUPERVISED)}")
    ...
    info.append(f"Queue: {len(models)} model(s) - {', '.join(m.upper() for m in models) or '(none)'}")
    if skipped:
        info.append(
    ...
    if not models and not skipped:
        ...
        warnings.append("All models appear complete; use --retrain-completed-models to force a full retrain.")

    if len(pairs) > 1 and int(getattr(args, "pair_embed_dim", 0) or 0) == 0:
        warnings.append(
            f"Multi-pair training ({len(pairs)} pairs) with pair_embed_dim=0 - consider pair_embed_dim: 16 in run.yaml."
        )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    sec = audit_settings_yaml_section_mismatches(raw, yaml_path=str(cfg_path))
                    errors.extend(sec.get("errors") or [])
                    warnings.extend(sec.get("warnings") or [])
                    arg_m = audit_args_vs_yaml_mismatches(args, raw, yaml_path=str(cfg_path))
    ...
    if bool(getattr(args, "training_memory", True)) and not getattr(args, "all_models", False):
        warnings.append("training_memory=true on a single-model run - global or per-model nudges may apply at startup.")
...
def build_config_preflight_report(args: Any) -> dict[str, Any]:
    ...
    try:
        models, skipped = resolve_models_to_train(args, apply_resume_filter=bool(getattr(args, "resume", False)))
        ...
        for m in models:
            ...
            if hasattr(m_args, "_training_features_report"):
                ...
                for k, v in report.items():
                    lines.append(f"      {k:20s}: {v['mode']!s:<6s} (source: {v['source']})")
    ...
    try:
        ...
        if len(_pairs) > 1:
            ...
            if _skipped:
                ...
                if _low:
                    lines.append(f"    ℹ Low coverage pairs ({len(_low)}): {', '.join(_low)}")  # noqa: RUF001
    except Exception:
        pass  # Non-blocking
