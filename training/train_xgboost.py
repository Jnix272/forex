    ...
    for combo in combos:
        ...
        if task == "classification":
            ...
            m = xgb.XGBRegressor(
                ...
                learning_rate=lr, eval_metric="rmse", verbosity=0, **params
            )
            m.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=False)
            preds = m.predict(X_val)
...
def main():
    ...
    _do_feature_importance = os.environ.get("XGB_FEATURE_IMPORTANCE", "1" if xgb_cfg.get("feature_importance", True) else "0") == "1"
    ...
    if args.folds > 0:
        ...
        for fold_i, (tr_idx, va_idx) in enumerate(walk_forward_splits(N, args.folds)):
            ...
            if args.task == "classification":
                m = xgb.XGBClassifier(
                    num_class=3, objective="multi:softmax",
                    eval_metric="mlogloss", verbosity=0, **xgb_params
                )
                ...
                m = xgb.XGBRegressor(
                    objective="reg:squarederror",
                    eval_metric="rmse", verbosity=0, **xgb_params
                )
                m.fit(Xtr, ytr, eval_set=[(Xva, yva)], verbose=False)
                preds = m.predict(Xva)
