...
def wrap_loader_prefetch(loader, args=None, *, prefetch: int | None = None):
    ...
    *only* way to overlap Zarr decompress + H2D with the GPU step - wrap
    ...
    ``batch_size`` x seq-len x feature-dim) contending the GIL for nothing -
...
class MemmapSequenceDataset(Dataset):
    ...
    def __init__(self, cache_path: str, indices: np.ndarray, scaler=None):
        ...
        self.use_zarr = ZARR and Path(cache_path).is_dir() and (Path(cache_path) / ".zgroup").exists()
    ...
    def __getitem__(self, idx):
        ...
        if self.use_zarr:
            ...
            y = float(self.y_zarr[real_idx])  # type: ignore[arg-type]
        else:
            X = np.array(self.X_mmap[real_idx], dtype=np.float32)
            y = float(self.y_mmap[real_idx])  # type: ignore[arg-type]
...
class ZarrStreamDataset(IterableDataset):
    ...
    def __init__(
        ...
        if self.use_zarr:
            ...
            if meta_file.exists():
                self._zarr_row_chunk = int(_json.loads(meta_file.read_text())["chunks"][0])
    ...
    def _decompress_block(self, block_idx, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr):
        ...
        if is_zarr and (end - start) == len(block_idx):
            ...
            yc_blk = np.array(y_cls_arr[start:end], dtype=np.float32) if y_cls_arr is not None else None
            pq_blk = np.array(pq_arr[start:end], dtype=np.float32) if pq_arr is not None else None
            ...
            if is_zarr:
                ...
                yc_blk = np.array(y_cls_arr.oindex[block_idx], dtype=np.float32) if y_cls_arr is not None else None
                pq_blk = np.array(pq_arr.oindex[block_idx], dtype=np.float32) if pq_arr is not None else None
            else:
                X_blk = np.array(X_arr[block_idx], dtype=np.float32)
                y_blk = np.array(y_arr[block_idx], dtype=np.float32)
                yc_blk = np.array(y_cls_arr[block_idx], dtype=np.float32) if y_cls_arr is not None else None
                pq_blk = np.array(pq_arr[block_idx], dtype=np.float32) if pq_arr is not None else None
    ...
    def _yield_block_rows(self, X_blk, y_blk, yc_blk, pq_blk, block_idx, rng):
    ...
    def __iter__(self):
        ...
        if self.shuffle_chunks and self.shuffle_buffer_size > 0:
            return self._iter_buffered(blocks, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr, rng)
        return self._iter_unbuffered(blocks, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr, rng)

    def _iter_unbuffered(self, blocks, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr, rng):
        for block_idx in blocks:
            blk = self._decompress_block(block_idx, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr)
            if blk is None:
                continue
    ...
    def _iter_buffered(self, blocks, X_arr, y_arr, y_cls_arr, pq_arr, is_zarr, rng):
        ...
        def _push(X_row, y_val, yc_val, pq_val, smp_idx):
            ...
            buf_X.append(X_row)
            buf_y.append(y_val)
            buf_yc.append(yc_val)
            buf_pq.append(pq_val)
            buf_idx.append(smp_idx)
            if len(buf_X) > cap:
