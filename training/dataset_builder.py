...
def _warmup_load_start(win_start: str, warmup_days: int = _FEATURE_WARMUP_DAYS) -> str:
    ...
    return (
        pd.to_datetime(win_start, utc=True) - pd.Timedelta(days=int(warmup_days))
    ).strftime("%Y-%m-%d")
...
_HOST_DEPS = (
    ...
    'collect_headlines_for_range',
    'load_historical_news_bundle',
    'SentimentPipeline',
    'align_labels_with_features',
    'compute_rl_reward_labels_regime',
...
def _ensure_bound() -> None:
    """Refresh host bindings (train_gpu defines some helpers after our import)."""
...
def _write_feature_schema_json(cache_path: Path, feature_names: list[str], args=None) -> None:
    ...
    if args is not None:
        _enforce_dataset_feature_schema(
            args, feature_names, cache_path, phase="final"
        )
...
def _enforce_dataset_feature_schema(
    ...
    *,
    ...
    for line in format_audit_warnings(
                        ...
                        **(
                            {
                                ...
                                "mask_enabled_missing": (
                                    v.get("mask_enabled_missing") or []
                                )[:20],
                            }
                            if k == "built_schema"
                            else {}
    ...
    if errors and gate:
        ...
        raise RuntimeError(
            f"Dataset feature-schema gate failed ({phase}): "
            + "; ".join(errors[:5])
            + hint
        )
    elif errors:
        ...
        try:
            ...
            if mod.startswith("polars"):
                num_cols = [
                    ...
                    or str(feature_frame[c].dtype) in (
                        "Float32", "Float64", "Int8", "Int16", "Int32", "Int64",
                        "UInt8", "UInt16", "UInt32", "UInt64",
                    )
                ]
                vals = (
                    feature_frame.select(num_cols).to_numpy()
                    if num_cols else np.empty((0, 0), dtype=np.float64)
                )
            else:
...
class ChunkResult:
    ...
    def __getitem__(self, key: Any):
        ...
        items = (
            self.X_seq, self.y_seq, self.diff_seq, self.pq_seq,
            self.y_cls_seq, self.close_seq, self.atr_seq, self.spread_seq,
            self.n_features, self.time_idx,
        )
        return items[key]
...
def _build_chunk(
    ...
    if hasattr(_t0, "to_pydatetime"):
        _t0, _t1 = _t0.to_pydatetime(), _t1.to_pydatetime()
    ...
    if "news_ok" in F.columns:
        ...
        if "no_trade_score" in F.columns:
            F = F.with_columns(
                pl.max_horizontal(pl.col("no_trade_score").cast(pl.Float64), news_nt)
                .alias("no_trade_score")
            )
        else:
    ...
    row_quality, row_drop_reasons, label_filter_counts, row_reason_masks = (
        _compute_row_quality_mask(bars_pd, feats_pd, labels)
    )
    ...
    _n_feat_cols = int(X.width) if hasattr(X, "width") else (
        int(X.shape[1]) if hasattr(X, "shape") and len(X.shape) > 1 else 0
    )
    ...
    x_index = pd.DatetimeIndex(
        pd.to_datetime(sidecar["timestamp_utc"].to_numpy(), utc=True)
    )

    row_reason_values = {
        name: mask.reindex(x_index).fillna(False).values.astype(bool)
        for name, mask in (row_reason_masks or {}).items()
    }

    row_quality = row_quality.reindex(x_index).fillna(False).values.astype(bool)
    ...
    close_seq, atr_seq, spread_seq = _market_bar_arrays_from_feats(
        feats_aligned, x_index, fe, seq_len
    )
...
def _build_multipair_dataset(
    ...
    if use_zarr:
        ...
        print(
            f"[Data] Zarr compressor=Blosc("
            f"cname={_cname}, "
            f"clevel={getattr(_compressor, 'clevel', 1)})"
        )
    else:
            ...
            with ProcessPoolExecutor(max_workers=_pw_workers) as _p_pool:
                ...
                for _wa in _batch:
                    ...
                    if not getattr(args, "_feature_schema_checked", False):
                        _partial = {
                            p: _all_worker_scalers[p][0]
                            for p in pairs
                            if _all_worker_scalers.get(p)
                        }
    ...
    if readiness_report.get("status") == "fail":
        raise RuntimeError(
            f"Pair Readiness Gate Failed. See {cache_path!s}_pair_readiness_report.json"
        )
        ...
        if use_zarr:
            if z_store is None:
                if getattr(args, "_resume_zarr", False):
                    ...
                    _zarr_create(
                        z_store, "X", shape=(0,) + X_seq.shape[1:], chunks=c0,
                        dtype=ZARR_FEATURE_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "y", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "diff", shape=(0,), chunks=(c0[0],),
                        dtype="uint8", compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "pq", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "y_cls", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "close", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "atr", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
                    _zarr_create(
                        z_store, "spread", shape=(0,), chunks=(c0[0],),
                        dtype=ZARR_LABEL_DTYPE, compressor=_compressor,
                    )
