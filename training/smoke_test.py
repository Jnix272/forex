...
def _make_dataset(cfg: dict[str, Any], device: torch.device) -> tuple[DataLoader, DataLoader]:
...
def _test_model(
    ...
    try:
        model.eval()
        dummy_x = torch.randn(cfg["batch_size"], cfg["seq_len"], cfg["n_features"], device=device)
        ...
        result.fwd_ok = loss_val == loss_val  # NaN check
    except Exception as e:
    ...
    try:
        ...
        scaler = (
            torch.amp.GradScaler("cuda") if use_amp and device.type == "cuda" and amp_dtype == torch.float16 else None
        )
    ...
    try:
        train_dl, _val_dl = _make_dataset(cfg, device)
        opt = torch.optim.AdamW(model.parameters(), lr=cfg["lr"])
        ...
        result.train_ok = (
            result.oom_skips == 0 and result.nan_skips == 0 and last_loss == last_loss  # not NaN
        )
    except Exception as e:
    ...
    result.passed = result.build_ok and result.fwd_ok and result.bwd_ok and result.train_ok and result.ckpt_ok
    return result
...
def _print_results_rich(
    results: list[ModelResult], total_s: float, cfg: dict[str, Any], device_str: str, use_amp: bool
) -> None:
    ...
    t = Table(show_header=True, header_style="bold cyan", border_style="dim", show_lines=True, expand=True)
    ...
    for r in results:
        ...
        oom_nan = ""
        if r.oom_skips:
            oom_nan += f"OOMx{r.oom_skips} "
        if r.nan_skips:
            oom_nan += f"NaNx{r.nan_skips}"
        if r.nan_grads:
            oom_nan += f"∇NaNx{r.nan_grads}"
    ...
    _console.rule(
        f"[{summary_style}]{n_pass}/{len(results)} models passed[/]  [dim]total {total_s:.1f}s[/]",
...
def _print_results_plain(results: list[ModelResult], total_s: float) -> None:
    ...
    print(f"\n{SEP}")
    print(
        f"  {'MODEL':<12} {'STATUS':<7} {'BUILD':<7} {'FWD':<6} {'BWD':<6} "
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gpu", action="store_true", help="Use GPU if available (default: CPU)")
    p.add_argument("--amp", action="store_true", help="Enable AMP mixed precision (requires --gpu)")
    p.add_argument(
        "--dtype",
        type=str,
        default="auto",
    ...
    p.add_argument(
        "--seq-len", type=int, default=DEFAULTS["seq_len"], help=f"Sequence length (default: {DEFAULTS['seq_len']})"
    )
    p.add_argument(
        "--n-features",
        type=int,
    ...
    p.add_argument("--verbose", action="store_true", help="Print per-model progress as each check runs")
    return p.parse_args()
...
def main() -> int:
    if not _TORCH:
        print("[ERROR] PyTorch is not installed. Install it from https://pytorch.org/get-started/locally/")
        return 1
    ...
    if args.gpu and torch.cuda.is_available():
        ...
        if args.gpu:
            _print("[yellow]No CUDA GPU found - running on CPU.[/]" if _RICH else "No CUDA GPU found - running on CPU.")
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    if _RICH and _console:
        _console.rule("[bold cyan]Forex Scaling Model - Pre-flight Smoke Test[/]", style="cyan")
        _console.print(
    ...
    with tempfile.TemporaryDirectory() as ckpt_dir:
        ...
        for i, name in enumerate(models_to_test, 1):
            prefix = (
                (f"[bold cyan][{i}/{len(models_to_test)}][/] Testing [cyan]{name.upper()}[/]...")
                if _RICH
                else f"[{i}/{len(models_to_test)}] Testing {name.upper()}..."
            )
            ...
            if _RICH and _console:
                if result.passed:
                    ...
                    _console.print(f"[bold red]FAIL[/]  [red]{result.error[:80]}[/]")
            else:
